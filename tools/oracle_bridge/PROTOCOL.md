...
| Field | Type | Disposition | Source | Note |
|---|---|---|---|---|
| `relics` | list<relic> | S | :326 | boss-relic pick. **Promoted from `I (S2 scope)` by S2.42.** The old reason ("the run terminates at act-1 boss combat rewards, before the boss chest", design §1.1 "Out") stopped being true at capture driver `b1.7.0`, which plays through the chest — and an `I` field is *never diffed*, so design §6's S2-G2 item 2 (a **zero-diff** boss-relic pick) was unachievable while this row said `I`, with no S2 ledger row owning the change. The relic ids are registry-**joined** today, so an unknown boss relic here fails translation loudly; the field is `fr.defer`red because it has no schema **storage** yet. The three offers live in `RunController.boss_chest` (`BossChestState`, `boss_chest.hpp`), which is transient — the translator emits `RunState`/`CombatState`, so landing it needs new `RunState` storage plus a `SCHEMA_VERSION` bump, a trace-container change and an oracle-adapter change. Storage owner: **S2.43** (`docs/s2-tasks.md` Deferred obligations) |
