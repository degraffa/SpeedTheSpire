...
| Explosive Potion THORNS model substitution | final-integrate (STS00509 discharge) | UNASSIGNED — whoever registers an owner-insensitive `onAttacked` power | Explosive Potion is modelled as THORNS in place of the game's pure/null-source NORMAL matrix — exactly right for every currently-registered ON_ATTACKED power (THORNS / CURL_UP / FLAME_BARRIER / ANGRY, all `info.owner != null`-gated in the Java), and wrong the day an owner-INsensitive `onAttacked` power (e.g. Act-2 Malleable, `MalleablePower.java` — no owner gate) is registered. The faithful fix is a `kDamagePure` flag bit + null-source sentinel in the DAMAGE item's flags word (bits 8+ free), which would also give the Fire Potion row's deliberately-unmodeled `applyEnemyPowersOnly` quirk (named at that row, above) its exact home |
