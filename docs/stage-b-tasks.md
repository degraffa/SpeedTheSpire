...
| Living Wall's generated COLORLESS transform-pool completion | B4.12 | B3.10c — the last missing colorless rows | B4.12 emits the event transform arrays directly from every registry card's color/rarity columns, so membership self-completes without another event-code edit as the mandatory missing colorless rows land — B3.10b and B3.11 have landed theirs; only B3.10c's Purity/Forethought remain. The live tier-2 test pins the generated shape and exact RED draw today. This is a temporary completeness dependency, **not permission to exclude or defer any colorless card**. |
...
**Note (B4.12 integration):** its two rows also complete B4.12's
metadata-derived Living Wall COLORLESS transform pool — no event-code edit
needed; see the Living Wall obligation row.
