...
| Lantern counter is used as a first-turn latch although the game leaves it at -1 | B5.2 exercise campaign | UNASSIGNED — relic engine owner before G7 | `b52_accept_20260729_70000_70049` STS70011 first diverges at seq 90/floor 9 (`relics[1].counter -1→0`), independently confirmed by acceptance seed STS71017. Lantern's Java uses a private `firstTurn` boolean, resets it at pre-battle, and never writes `AbstractRelic.counter`; the sim registry instead seeds `initial_counter: 0` and uses that observable slot as the latch. This is a product divergence, not an accepted live override. Exact triage: [`b52_exercise_triage.md`](../tools/oracle_bridge/driver/b52_exercise_triage.md) |
| Replay HAND_SELECT command can name a slot absent from the sim's open screen | B5.2 exercise campaign | UNASSIGNED — replay harness owner | `b52_accept_20260729_70000_70049` STS70022 is zero-diff through seq 29, then `choose 0` cannot be mapped because the capture and sim expose different HAND_SELECT rows. Classify/reduce the choice-surface disagreement before calling it a mechanic; the prefix is clean and this is currently a harness stop. Exact triage: [`b52_exercise_triage.md`](../tools/oracle_bridge/driver/b52_exercise_triage.md) |
| Whole-run replay command-map gaps exposed after zero-diff prefixes | B5.2 acceptance campaign | UNASSIGNED — replay harness owner | `b52_accept_locked_20260729_71000_71049` has five harness stops after zero-diff prefixes: captured `play` with no mappable card index (STS71009/71022), reward-screen potion use (STS71015), and shop `choose` after the sim has returned to `MAP_CHOICE` (STS71030/71040). Keep all five out of replay-clean action totals after their stop; reduce/map them rather than accepting mechanics. Exact triage: [`b52_exercise_triage.md`](../tools/oracle_bridge/driver/b52_exercise_triage.md) |
| Spike Slime damage can consume block without the final unblocked HP in sim replay | B5.2 exercise campaign | UNASSIGNED — combat engine owner before G7 | `b52_accept_20260729_70000_70049` STS70023 first diverges at seq 35/floor 2: both sides consume block 5 against a 6-damage Spike Slime attack, but the capture reaches HP 41 and the sim remains at HP 42. No standing deviation covers the shape. Product-divergence candidate pending a minimal reducer; not accepted. Exact triage: [`b52_exercise_triage.md`](../tools/oracle_bridge/driver/b52_exercise_triage.md) |
| Wheel of Change result acknowledgement advances reward pool/RNG one command early | B5.2 exercise campaign | UNASSIGNED — event engine owner before G7 | `b52_accept_20260729_70000_70049` STS70037 first diverges at seq 82/floor 10: the sim has entered `COMBAT_REWARD`, popped an uncommon relic and advanced `relicRng`, while the capture is still on the prize/result acknowledgement; the one-entry pool/RNG lead persists. Product-divergence candidate pending a state-machine reducer; not accepted. Exact triage: [`b52_exercise_triage.md`](../tools/oracle_bridge/driver/b52_exercise_triage.md) |
| Tiny House Neow payout misses the selected card and gold in whole-run replay | B5.2 acceptance campaign | UNASSIGNED — boss-relic/run-layer owner before G7 | `b52_accept_locked_20260729_71000_71049` STS71018 first diverges at seq 5/floor 0: after the Tiny House reward choices the capture has the selected Body Slam, then 50 additional gold, while the sim has neither. Product-divergence candidate pending a minimal reducer; not accepted. Exact triage: [`b52_exercise_triage.md`](../tools/oracle_bridge/driver/b52_exercise_triage.md) |
| Gambling Chip private activation latch leaks through observable relic counter | B5.2 acceptance campaign | UNASSIGNED — relic engine owner before G7 | `b52_accept_locked_20260729_71000_71049` STS71033 first diverges at seq 10/floor 1: capture `Gambling Chip.counter == -1`, sim `== 1`, and the difference persists. The Java latch is private state, not `AbstractRelic.counter`; the current `RelicSlot.counter` implementation is observably wrong even if its choice effect is correct. Product divergence; not accepted. Exact triage: [`b52_exercise_triage.md`](../tools/oracle_bridge/driver/b52_exercise_triage.md) |
| Calling Bell Neow onEquip grid/pool application is absent or mistimed in whole-run replay | B5.2 acceptance campaign | UNASSIGNED — boss-relic/run-layer owner before G7 | `b52_accept_locked_20260729_71000_71049` STS71039 first diverges at seq 2/floor 0 with 100 fields: the capture has opened/advanced the curse grid, added Curse of the Bell and consumed three relic-pool entries while the sim remains at pre-equip Neow state; the harness then stops downstream. Product-divergence candidate pending a minimal reducer; not accepted. Exact triage: [`b52_exercise_triage.md`](../tools/oracle_bridge/driver/b52_exercise_triage.md) |
