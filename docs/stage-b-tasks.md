...
|---|---|---|---|
| **Sharp Hide THORNS retaliation on the killing blow** | TE.1 (training ledger) | **S3.44** `[x]` | Live game applies THORNS retaliation even when the damage kills its source (source-dead THORNS is not cancelled); the sim's pump drops the queued retaliation at combat-over. Witness STS420252 (te1_survival_b160 cohort): hp 22 (sim) vs 26 (game) to terminal. Reproducer promoted by TE.1; triaged open-product-divergence, not standing. Fix is a pump/combat-over ordering change — core semantics, needs its own task with fixture impact assessed, not a drive-by. **DISCHARGED by S3.44, 2026-09-03 — and the row's own defect was already closed before it, by two later S2 tasks this table never re-owned.** S2.43's four-arm `clearPostCombatActions` survivor set (`86fc2be`) put `actionType == DAMAGE` back in the victory-terminal survivor set — its derivation names "the Guardian's Sharp Hide onUseCard retaliation" and seed STS431342 by name — and S2.49 (`d57e077`) exempted THORNS from the attacker-side cancel; both landed **after** TE.1 recorded this row on 2026-08-03. The witness capture itself is gone (the whole `te1_survival_b160_20260803_420000_420499` group, reproducer included, is no longer under the §7.3 data root), so S3.44 re-witnessed the family from a **21-instance cohort of the identical shape** found across the 184 on-disk captures that carry a Sharp Hide power — STS431342 among them — all replaying clean. What S3.44 actually changed is the **residual** half of the same seam: the terminal resolver drained a snapshot, so an action a survivor queued while resolving was never popped; it now drains the ring, which is what makes the Heart's `BeatOfDeathPower` retaliation land. See the S3.44 Log in [s3-tasks.md](s3-tasks.md) |
...
| Sacred Bark potency at EVENT acquisition/use sites | B3.27 | G7 (B5.2–B5.5) | **DISCHARGED.** The five BOSS `onEquip` bodies (Pandora's Box / Tiny House / Astrolabe / Empty Cage / Calling Bell) formerly sharing this row were already discharged by Wave-C track 2 (`wave-runlayer` S1); their full history is archived in the B4.11/B4.12/B4.13 Logs. G7 closes the Sacred Bark half: immutable base potion definitions now receive the equivalent x2 at every live use/trigger site — the central combat data/native path, direct run-layer Fruit Juice and Blood Potion use, and both Fairy in a Bottle revive paths, including event damage. Lab and The Woman in Blue only grant a potion *identity* through `claim_reward`, so acquisition correctly has no potency read. Tests pin ordinary data potions and the STS302374 Fruit Juice witness; the final b1.5.3 replay campaign exercises the implementation under oracle comparison. |
| `colorlessCardPool` is shuffled IN PLACE by `returnColorlessCard` | B4.13 | UNASSIGNED — but the named consumer turned out not to be one | `AbstractDungeon.returnColorlessCard(rarity)` (`AbstractDungeon.java:1100-1113`) JDK-shuffles the persistent `colorlessCardPool.group` before picking, so the new ORDER survives into the next reader of that list. Match and Keep is the only Act-1 caller and the port shuffles a local copy. **B4.8 corrects this row's forward-looking half:** it named "a shop with colorless slots" as the consumer that would observe the persisted order, and the shop now exists and does **not**. `getColorlessCardFromPool` reaches `CardGroup.getRandomCard(true, rarity)` (`CardGroup.java:509-524`), which filters the group into a local `tmp` and **`Collections.sort`s it** before indexing, so the source order is discarded on every read. Nothing in Act 1 observes the persisted order at all — `transformCard`'s COLORLESS branch reads the untouched `srcColorlessCardPool` (`:998-1014`). The row stays open only against a future reader of the UNSORTED whole-pool view. **RE-CHECKED 2026-07-28 (Track P, wave-2): STILL NO READER — the row's verdict stands, and the sweep is now exhaustive rather than by-name.** Every read of the LIVE `colorlessCardPool.group` in the tree was enumerated (`AbstractDungeon.java:1020, 1102, 1107, 1117, 1185`; `:1582`/`:1588` go through `getRandomCard`). Four of the five are not readers of a PERSISTED order: `:1102`/`:1107` are `returnColorlessCard`'s own loops, which shuffle first; `:1117` is the no-arg `returnColorlessCard()` (called only from `CardRewardScreen.java:710`), whose `Collections.shuffle` takes NO seeded Random at all, so no stream observes it; and `:1185` is `initializeCardPools` copying the live pool into `srcColorlessCardPool` — preceded in the SAME method by `colorlessCardPool.clear()` (`:1141`) and a rebuild from `CardLibrary.cards` (`addColorlessCards`, `:1203-1209`), so an act transition **resets** the order and a previous act's shuffle can never carry into it. The fifth, `:1020`, is the only genuine unsorted whole-pool reader — `returnTrulyRandomCardFromAvailable`'s COLORLESS arm — and it is **unreachable from `transformCard`**, which routes COLORLESS to the twin that reads `srcColorlessCardPool` instead (`:861-864` → `:1007-1014`); its only other caller is `GridCardSelectScreen.java:266`, an upgrade PREVIEW. Engine side re-swept too: the shop's colorless slots reach `getColorlessCardFromPool` (`shop.cpp:365-368`), the sorted view, and `draw_colorless_uncommon` (`shrines.cpp:112-125`) still shuffles a LOCAL copy. *(Method note: `:1018`'s `case 1:` = COLORLESS is no longer an inference — `AbstractDungeon$1`, the stripped switch-map class, was recovered from the shipped jar in the same task and decodes it; see `RECOVERED-INNER-CLASSES.md` at the decompiled tree root.)* Row stays OPEN in the same narrowed form; next re-check should redo the `:1020` reachability question if `transformCard` ever grows a caller that passes a colorless card to the wrong twin. **DISCHARGED by S2.32 (2026-08-09) — the reader arrived and the order is now MODELLED.** The reader is not a whole-pool VIEW but `returnColorlessCard` ITSELF, called repeatedly within one act: each call's `Collections.shuffle` PERMUTES the order the previous call left, so from the SECOND same-act call on the picked card depends on the persisted order — and Knowing Skull's repeatable CARD purchase (KnowingSkull.java:152-161, several buys in one visit) makes that reachable in ordinary play, where Act 1's single M&K call never was. Landed as `RunController.colorless_order` (transient CONTROLLER state, not RunState: the game itself rebuilds the pool to library order in every dungeon ctor — `initializeCardPools`, the `:1141`/`:1203-1209` reset this row already proved — so the order is genuinely session-transient), initialized at run_begin and every act crossing, shuffled in place by the shared `event_draw_colorless_uncommon` (event_framework.cpp; the old local-copy `draw_colorless_uncommon` in shrines.cpp is deleted and M&K now goes through the shared door — its first-draw-of-the-act bytes are unchanged, which is why no golden moved). Classified HIDDEN in `byte_class.hpp`. Pinned by `KnowingSkull.SecondCardBuyReadsTheFirstBuysPoolPermutation` (tests/city_one_timers_test.cpp) |
| Fusion Hammer / Coffee Dripper campfire-option locks | B3.27, B4.9 | `wave-combat` (Track 1 stage 2) | **DISCHARGED**, in the same wave as the shared `energyMaster` half B4.9 was waiting on. `build_rest_menu` now runs the real veto sweep (`CampfireUI.initializeButtons`, `CampfireUI.java:87-93`): the whole button list is built first, then every button is offered to every relic in acquisition order and the first refusal clears its `usable`. Sweeping the full list — not just the two base buttons — is the Java's shape and is what keeps a future refusing relic correct. The relics' own `updateUsability(false)` calls (`SmithOption.java:24-27`, `RestOption.java:43-48`) are **cosmetic** and are deliberately not modelled as the disable. **The consequence nobody had modelled is also landed:** when no button is usable the game completes the room on the spot (`CampfireUI.java:97-104`, `waitTimer = 0`, `phase = COMPLETE`), and that is reachable in S1 with a *single* relic — Coffee Dripper plus a deck with nothing upgradeable — so rest-room entry checks it and returns to `MAP_CHOICE` rather than parking the run on a menu with no legal action. `RecallOption` is appended after the sweep and is Act-4-gated, so it is absent rather than modelled as always-off **(superseded 2026-07-28, wave3-followup: RecallOption is now MODELLED -- `RestOptionKind::RECALL` appended after the sweep under `kFinalActAvailable && !(keys & kKeyRuby)`, taking it records the Ruby Key and completes the room, and the auto-complete above is therefore only reachable once the key is taken; triage in wavec_track2_replay_triage.md)** |
...
| Windows CI job | build effort | **DISCHARGED by S3.66** (docs/s3-tasks.md) | the proposed workflow is retooled and landed: `ci.yml`'s `windows` job builds `win-debug` under LLVM **22.1.8, pinned exactly** (the version already verified here), with the same build+real-run evidence the Linux job now runs. Local mirrors on both hosts pass; the green-run-on-a-real-push confirmation is the orchestrator's to close from the Actions API on the landing push — see the S3.66 Log |
...
| The Courier (shop relic) | B3.25 | B4.8 → **UNASSIGNED for the restock half**; see the blocker | **PRICE HALF DISCHARGED by B4.8:** the `x0.8` discount is applied at shop init in the Java's order (and is therefore overwritten, not compounded, by a Membership Card at that call site — reproduced, not corrected), and its purge-cost branch is live in both `shop_purge_cost_at_init` and `shop_purge_cost_after_purge`, the latter with the `0.8f * 0.5f` product the Java spells there. **The RESTOCK half stays deferred, and it is BLOCKED, not merely unscheduled:** `ShopScreen.purchaseCard`'s replacement draws `getCardFromPool(rollRarity(), type, false)` — `useRng=false` means `MathUtils.random`, libGDX's **unseeded global**, not `cardRng` (`ShopScreen.java:615-617`), so the replacement card's identity is not reproducible from a seed at all. The rarity roll before it, and the relic/potion restocks (`StoreRelic.java:105-112`, `StorePotion.java:86-89`), ARE seeded; whoever re-owns this should decide what a deterministic simulator does about an unseeded identity before writing any of it. B4.8's runbook §4 asks the operator to capture a Courier shop specifically to measure what the restock costs the seeded streams. **RESTOCK HALF DISCHARGED by Track C wave2-capture (2026-07-28/29).** The decision recorded at Wave-C stands implemented: every SEEDED half is modelled faithfully (shop.cpp/shop.hpp Courier block — colored rarity roll cardRng +1 + setPrice merchantRng +1; colorless merchantRng rare roll + cardRng identity + setPrice, fully seeded and purchasable; relic rollRelicTier + END-pop + getNewPrice; potion returnRandomPotion + getNewPrice; restock prices carry NO A16 pass and no sale halving), and the unseeded colored-card identity is the **permanent named deviation**: the slot restocks as `kShopRestockedUnknownCard`, priced off its seeded rarity, OFF the legal mask and refused byte-stable (`CourierRestock.*` guard tests, the potion_use_implemented precedent). MEASURED live: campaign `wave2cap_courier_buy_20260728T224858Z_claude01`, 13 purchases over 4 scripted Courier shops — colored = cardRng +1 / merchantRng +1 exactly; colorless = cardRng +1 / merchantRng +2; relic = merchantRng +2 + one pool end-pop; potion = merchantRng +1 / potionRng +2 (+1 rejection observed once); no other stream moved, blizz never moved. `replay_run_diff --shop` over the capture: stock clean 5, purchase walks clean 5, ZERO divergences. Full table + procedure: tools/oracle_bridge/driver/wave2cap_capture_runbook.md §4. **"PERMANENT" SUPERSEDED 2026-09-03 by S3.24** — the deviation is gone: the restocked colored identity now draws from a dedicated derived stream on both sides (`courier_restock_stream` in shop.hpp, `patches/CourierRestockSeedPatch` in the fork), `kShopRestockedUnknownCard` and its buy-refusal are deleted, and the slot is on the legal mask. The stream motion measured above is UNCHANGED, because the new stream is constructed at the draw and never stored. See the S3.24 row in [s3-tasks.md](s3-tasks.md) |
...
| `b14_accept2` obtain-race capture-fidelity triage | B1.3 | B5.2 | **DISCHARGED by B5.2.** The named reproducer is promoted at `tests/golden/oracle_reproducers/b14-living-wall-obtain-race/`: `b14_accept` STS00009 floor 4 Living Wall — the transform removes a Defend_R immediately (miscRng 0→1) but `ShowCardAndObtainEffect` stores Dark Embrace in its constructor and grants it only from `update` after the animation (`ShowCardAndObtainEffect.java:30-45,94-108`), so seq 40-47 show deck 11 and seq 48 (next floor) shows 12. `b14_accept2` completed the same seed sweep but did not take that branch, so it is not misrepresented as a second witness. `replay_run_diff --event` recognises the shape NARROWLY (every differing field must be `master_deck_count` or a `master_deck[i]` at/past the seeded deck's end) and reports `RACE` instead of a divergence, so no stream/pool/`event_flags` diff can hide behind it. This is a capture-fidelity classification, not a live-overrides-Java mechanic claim; B1.4's acceptance remains unaffected |
| Lantern counter is used as a first-turn latch although the game leaves it at -1 | B5.2 exercise campaign | G7 | **DISCHARGED by G7.** Lantern's first-turn decision is now derived from the first turn-start call (`CombatState.turn == 0`), exactly replacing its Java-private `firstTurn` field without touching the oracle-visible counter; `initial_counter` is back to -1. The focused first/second-turn regression pins both the energy and counter, and both original witnesses now replay clean. Exact final replay table: [`b52_exercise_triage.md`](../tools/oracle_bridge/driver/b52_exercise_triage.md#g7-supersession--every-open-row-resolved) |
| Replay HAND_SELECT command can name a slot absent from the sim's open screen | B5.2 exercise campaign | G7 | **DISCHARGED by G7.** CommunicationMod numbers the filtered selectable-card group while `advance(CHOOSE)` names a full-hand slot; the mapper now translates the captured ordinal to the Nth `can_choose` full-hand entry and fails loud when none exists. A Dual Wield-shaped regression pins the non-identity mapping; STS70022 and the larger preserved campaigns now replay through it clean. |
| Whole-run replay command-map gaps exposed after zero-diff prefixes | B5.2 acceptance campaign | G7 | **DISCHARGED by G7.** Combat `play 0` now names Java's tenth hand slot; potion use is phase-independent for the top-panel potions legal outside combat; reward/shop/chest `proceed` overlays are deferred until a map node actually moves; and combat discard/exhaust grids map by captured card identity. All five named acceptance stops and all corresponding B5.3 stops now replay to their next real boundary or terminal with zero state diff. |
| Spike Slime damage can consume block without the final unblocked HP in sim replay | B5.2 exercise campaign | G7 | **DISCHARGED by G7.** The damage pipeline was correct; the preceding Gremlin Visage Weak was not. Its `WeakPower(..., isSourceMonster=false)` must NOT arm the just-applied duration latch, so it expires at the player's end turn before Spike Slime attacks. Queued apply-power items now carry that source bit, a focused Visage regression pins the expiry, and STS70023 replays clean. |
| Wheel of Change result acknowledgement advances reward pool/RNG one command early | B5.2 exercise campaign | G7 | **DISCHARGED by G7.** The event now has distinct spin, result, and leave states: the first command selects/reveals, the acknowledgement applies the result and only then opens any reward, and the final acknowledgement leaves. Every outcome has timing assertions; STS70037 no longer advances its relic pool or RNG early and replays clean. |
| Tiny House Neow payout misses the selected card and gold in whole-run replay | B5.2 acceptance campaign | G7 | **DISCHARGED by G7.** Tiny House's nested card reward now returns to its still-live item reward, preserving the selected card before the remaining gold/potion/card payout steps. The Neow regression walks the complete nested screen sequence and STS71018 replays clean. |
| Gambling Chip private activation latch leaks through observable relic counter | B5.2 acceptance campaign | G7 | **DISCHARGED by G7.** The once-per-combat decision is derived from the first post-draw hook (`CombatState.turn == 1`); the Java-private `activated` field is no longer mirrored into `RelicSlot.counter`, which remains -1. Hook and combat-start regressions pin the screen, once-only behavior, and counter; STS71033 replays clean. |
| Calling Bell Neow onEquip grid/pool application is absent or mistimed in whole-run replay | B5.2 acceptance campaign | G7 | **DISCHARGED by G7.** Calling Bell now opens its choice-free curse confirmation grid first; only confirmation adds the curse and consumes the three relic tiers, after which the mandatory reward sequence advances one row at a time. The live driver suppresses the misleading `proceed` while those mandatory Neow relic rows remain. Neow/equip regressions pin both stream/pool timing and the grid, and STS71039 replays clean. |
| Infernal-Blade-generated Blood for Blood cost model (`cost_now` only; end-of-turn reset restores 4, not the game's reduced base) | B3.6 | G7 | **DISCHARGED by G7.** `updateCost(-1)` now reduces the saved combat-persistent base while preserving the current `cost - costForTurn` delta. An Infernal-Blade-generated zero-cost Blood for Blood therefore remains zero for that turn and resets to the correctly reduced base (3, or 2 when upgraded), with the hidden base stored in the existing saved-cost flag payload; no schema change. A focused generated-card/damage/reset regression pins the formerly deferred corner. |
...
| Fork redeploy + a bottle-taking capture (validate the `in_bottle_*` boundary end-to-end) | wave-runlayer S3 | **next capture-campaign owner** | the fork SOURCE change (`GameStateConverter.convertCardToJson`, PROTOCOL §3.13) and the translator mapping are landed and unit-tested, but no deployed jar emits the field yet and no existing capture takes a bottle, so the master-deck bottle bits have never been compared against the live game. Needs: rebuild + redeploy the fork (blocked on the game install, owned by the G6 campaign agent at stage time), then one capture that claims a Bottled relic and picks a card (an elite/chest reward or a shop purchase), replayed through `--replay` — note the grid session maps bottle-grid rows in REVERSE master-deck order (`command_map.hpp`; getCardsOfType's addToBottom is a prepend, CardGroup.java:1052-1058 → :459-461), which only a real capture can prove. **DISCHARGED by Track C wave2-capture (2026-07-28/29).** Fork rebuilt from this tree's source and redeployed (new pin `ADDE86099E54EB4E1313BA32802DDFE4CD89FBCE20F369E9D57E2C36595BF886`, determinism-checked; the b45 pin `7DC814AD…` is the pre-`in_bottle_*` build), preflight gate passed. SEVEN bottle-taking captures across `wave2cap_roundA2_20260728T224858Z_claude01` (STS00241/04888/04925/05143/06578, Bottled Flame) and `wave2cap_roundA3_…` (STS03244/03352, Bottled Lightning) — all chest claims whose mandatory grid the policy drove, all strict-validated, all translating with zero unknown-field errors (the `in_bottle_*` keys land on the deck walk). THE REVERSE-ORDER CLAIM IS **PROVEN**: STS04925's `--replay` re-drove the whole bottling (game grid = attacks in exact reverse deck order; `choose 1` = Sword Boomerang) and diffed ZERO on 91 records through and past it, master-deck flags included; promoted as `ReplayCommandMap.ABottleGridSessionSnapshotsTheLegalIndicesDescending`. Residue, named: the other six runs' replay windows stop at their first `SHOP_ROOM` screen (the command-map arm Track H is adding concurrently) with zero diffs to each stop — re-run `--replay` over them when it lands (the two Lightning runs are the SKILL-grid order evidence in waiting); no Bottled Tornado was reachable (its canSpawn needs a master-deck POWER — searched, reported, unclaimed). Procedure + triage: tools/oracle_bridge/driver/wave2cap_capture_runbook.md |
...
| Translator: real `act_boss` | B1.5, B4.3 | UNASSIGNED | **DISCHARGED by `wave2-harness` stage 3.** The join is the ENCOUNTER registry's own `encounter_by_game_id`, not a table of spellings: `AbstractDungeon.bossKey` is the same string `encounters.yaml` keys on ("The Guardian" / "Hexaghost" / "Slime Boss", `Exordium.initializeBoss`, `MonsterHelper.getEncounter`), so an unknown key — or a real encounter that is not a BOSS row — aborts like every other id join. The stored value is the **EncounterId**, because that is the space the run layer already speaks (its `boss_list[]` holds encounter game_ids and `enter_combat` takes one); a MonsterId would not survive a two-monster boss. **HONEST LIMIT, and a new obligation:** nothing in `src/engine` writes `RunState.boss_ids` — the run layer keeps the act boss in `RunController.lists.boss_list[]` and never mirrors it — so `--replay` neutralizes the field the way it already neutralizes `map[]`, and it comes back the moment `run_begin` records `boss_list[0]`. That mirror is a RUN-LAYER change and is re-owned to the run-layer owner, not closed here. **MIRROR DISCHARGED by `wave3-followup` (2026-07-28):** `run_begin` records `encounter_by_game_id(boss_list[0])->id` into `boss_ids[0]` (the same EncounterId join and space the translator writes), the `--replay` neutralization is REMOVED, and the differ compares the field for real — the whole 49-artifact corpus (b45+b47 twelve, G6-main thirty, bottle seven) re-ran with zero `boss_ids` divergence anywhere and verdicts byte-identical. Pinned by `RunBegin.BossIdsMirrorsTheRolledActBoss` |
| Bit-exact oracle for the raw monsterRng monster / elite / boss lists | B3.12 | B5.2 | **DISCHARGED by B5.2.** The oracle fork now emits the three raw shuffled lists as `encounterLists`; strict artifact validation and translation reject malformed/unknown rows; `encounter_list_oracle` independently rebuilds `run_begin(seed, ascension)` and compares all three lists plus the post-construction `monsterRng` state. Every seed in the independent 50-seed acceptance campaign `b52_accept_locked_20260729_71000_71049` passed the list comparison with zero diff, against fork jar SHA-256 `9D390597F37D1CDF4811609800E399C1B6750337DBF62D80D4C1E14C640D6BDE` |
...
| **Nine pre-existing out-of-range Java citations, repo-wide** | integration-15 citation audit | **DISCHARGED — Track P wave-2 (registry/docs half, 2026-07-28) + wave2-engine `9561c8e` (the 22 handed-off src/include/tools/tests lines, 2026-07-28); nothing remains open (S3.65 re-verified 2026-09-03)** | `RupturePower`, `DexterityPower`, `FrailPower`, `Clash`, `HeavyBlade`, `Torii`, `TungstenRod`, `LizardTail`, `MagicFlower` each carry a `File.java:line` citation that does not resolve in `D:\STS_BG_Mod\SlayTheSpireDecompiled`. Found while fixing the two citations integration-15's own merge broke, and **deliberately left alone** — they predate that integration and are a separate repo-wide condition, not something those branches introduced. Fixing them is comment/provenance-only and needs each cited method **re-read in full**, not line-shifted: the two integration-15 fixed both had *correct prose and wrong numbers*, so a mechanical offset would have looked right and been wrong. **PARTIALLY DISCHARGED 2026-07-28 (Track P, wave-2) — every citation re-derived, the registry/docs half LANDED, the source half HANDED OFF.** All nine methods were re-read in full. **The warning was right and understated it:** there is no `+1000` offset anywhere in these nine. Implied offsets are +1167 (Torii), +1213 (Tungsten Rod), +698 (Magic Flower), +645 (Lizard Tail), +379 (Heavy Blade), +140 (Clash), +29/+28 (Rupture — *not even self-consistent inside one file*), +20 (Dexterity), and **none at all** for Frail, whose defects are wrong-construct-in-range. A mechanical shift would have produced nine wrong answers. **PROSE VERDICT: 9 of 9 MATCH — no stop-the-line finding.** Every citing sentence describes what the Java actually does; only the numbers were wrong. Four prose *incompletenesses* are recorded rather than treated as mismatches (Lizard Tail's `setCounter` is GUARDED — any argument but `-2` is a silent no-op, now stated in `relics.yaml`; Magic Flower's guard omits the `currMapNode != null` conjunct, harmless in S1; Heavy Blade's Strength restore is an INTEGER `/=`, so a value not divisible by `magicNumber` is permanently truncated — the real game's known quirk, which no prose claimed otherwise about). One decompiler artefact is now annotated in place: `DexterityPower.java:87-89` renders as `float f; … if (f < 0.0f)`, reading an unassigned local — it would not compile; the bytecode tests `blockAmount < 0.0f`, i.e. the floor-0 the engine implements. **LANDED HERE (10 lines, provenance strings and comments only, no mechanic touched):** `powers.yaml:165` Rupture `:60-66`→`:32-37`; `powers.yaml:226` Dexterity `:106-111`→`:86-92`, `canGoNegative :31`→`:35`, `POWER_ID :14`→`:17`; `powers.yaml:292` Frail `:22-64`→`:16-61` and `:40-52`→`:40-49`; `cards.yaml:274` Clash `:176-181`→`:36-41`, `:184-194`→`:44-55`, `:172`→`:32`, `:198-203`→`:58-63`; `cards.yaml:338` Heavy Blade `:418-423`→`:39-44`, `:426-447`→`:47-68`, `:413-414`→`:34-35`, `:450-457`→`:71-78`; `relics.yaml:834` + `:1051` Magic Flower `:728-735`→`:30-37`; `relics.yaml:848` Torii `:1197-1205`→`:31-38`; `relics.yaml:918` Lizard Tail `:680-690`→`:36-45` and `:672-678`→`:28-33`; `relics.yaml:1044` Tungsten Rod `:1238-1245`→`:26-32`; `monsters.yaml:630` Frail ctor `:25-41`→`:24-37`; and this file's `:1161` Frail `:40-52`→`:40-49`. **HANDED OFF — 22 lines under `src/**`, `include/**`, `tools/**`, `tests/**`, which this track is read-only on** (Track E owns `src/engine`, Track H owns `tools/oracle_bridge`); the exact old→new list is in Track P's report and every one is a comment, so none can change behaviour. **Useful shortcut for whoever lands them: the repo ALREADY CONTAINS the correct citation for four of the nine** — Torii (`event_framework.cpp:542`), Tungsten Rod (`:554`), Lizard Tail (`:621`) and Magic Flower (nine sites) each have a verified-correct form living beside the broken one, so those need no judgement, only adoption. That split-brain is also evidence the bad numbers came from ONE bad authoring batch, not from drift. **Deliberately NOT edited: `docs/stage-b-log.md:965, :977, :1780`** — append-only archive; their corrections are Heavy Blade `:426-435`→`:47-56`, Clash `:184-194`→`:44-55`, Frail `:25-54`→`:24-37`, recorded here instead of rewritten there. **TRAP FOUND WHILE LANDING — a "comment-only" registry edit CAN break the suite, and did.** `tools/oracle_bridge/driver/cards_sidetable.json` pins `_provenance.source_sha256` of `registry/cards.yaml`, and `test_oracle_campaign.py::CardSideTableTest` compares the committed table against a fresh generation — so touching a `provenance:` STRING in `cards.yaml` fails the suite until the table is regenerated (`python tools/oracle_bridge/driver/gen_cards_sidetable.py --registry registry/cards.yaml --out tools/oracle_bridge/driver/cards_sidetable.json`, the command the file's own `_provenance.regenerate` gives). Regenerated in this commit; the diff is **the hash line and nothing else**, which is itself the proof the edit was provenance-only. Checked: `cards.yaml` is the ONLY registry file with a pinned artifact — `powers.yaml`, `relics.yaml`, `monsters.yaml`, `a20.yaml` have none, and `provenance` is otherwise validated for presence by `stsgen/loader.py:55` and never emitted into generated code. Whoever lands the 22 handed-off lines does not hit this (none are in `cards.yaml`), but the next editor of `cards.yaml` will. The 22 handed-off lines are LANDED: **FULLY DISCHARGED 2026-07-28** — wave2-engine 9561c8e (addendum 2) applied every one in the engine-owned files, each spot-verified against the decompiled tree before editing (plus the MagicFlower `:31-38`→`:30-37` normalization at three further sites its own stage-1 comments had copied forward). Nothing remains open |
| `a20.yaml` row 4 cites `tackleDmg = 10`, a dead `DamageInfo` | integration-11 | B4.15 follow-up | literally accurate (`SlimeBoss.java:94-96`) but `tackleDmg` is `damage.get(0)` and SlimeBoss never reads it — only `damage.get(1)` at `:137`/`:144`. `monsters.yaml` is right to omit it. Not a document conflict. **DISCHARGED 2026-07-28 (Track P, wave-2).** Resolution landed as recorded: `registry/a20.yaml` row 4 now carries a block comment stating that the asymmetry with `monsters.yaml` is DELIBERATE and why — the A20 file is the ascension LEDGER (it records what the `>= 4` branch assigns, dead assignment included) and `monsters.yaml` is the BEHAVIOUR table (it records what the monster does), so the two answer different questions and neither is wrong. The `provenance` string carries the same fact inline for anyone who reads only the row. Re-verified against the Java before landing: `SlimeBoss.java:101-102` adds `tackleDmg` then `slamDmg`, and the only two `damage.get` reads in the whole class are `:137` and `:144`, both `get(1)` — `damage.get(0)` is never read, so the A4 `tackleDmg = 10` is genuinely dead in the game. No mechanic changed |
| **STOP-THE-LINE — Mayhem plays the WRONG CARD: the engine collapses a queue level the Java has** | Track P jar re-extraction, wave-2 (2026-07-28) | **DISCHARGED** — wave2-engine 596f49e, 2026-07-28 | `MayhemPower$1` — the anonymous action `MayhemPower.atStartOfTurn` queues per stack (`MayhemPower.java:37`) — was **recovered from the shipped `desktop-1.0.jar`** and now sits in the decompiled tree beside its outer class (`MayhemPower.class` byte-identical in both jars; `RECOVERED-INNER-CLASSES.md`). Its ENTIRE body is `this.addToBot(new PlayTopCardAction(AbstractDungeon.getCurrRoom().monsters.getRandomMonster(null, true, AbstractDungeon.cardRandomRng), false)); this.isDone = true;`. **The reconstruction from DistilledChaosPotion/Havoc was EXACTLY RIGHT on the effect** — `PlayTopCardAction`, `exhausts = false`, one `cardRandomRng` roll deferred to drain time. **It was wrong on QUEUE DEPTH, and that is observable.** The Java's queued action does NOT play a card: it only ROLLS the target and `addToBot`s the `PlayTopCardAction` — a SECOND queue level. `addToBot` is `actions.add(action)`, a plain append (`AbstractGameAction.java:64-66` → `GameActionManager.java:96-100`), and `getNextAction` pops from the front (`:185-187`), so that play lands **behind everything already queued** — including the turn's own draw, which step 6 queues at `GameActionManager.java:361`, sixteen lines after `applyStartOfTurnPowers` at `:345`. **So the game plays the draw pile's top card AFTER the five-card draw has taken the first five; the engine plays it BEFORE.** With draw pile `[A,B,C,D,E,F,…]` the game draws A–E and plays **F**; `power_mayhem.cpp` (which `add_to_bottom`s a `PLAY_CARD` directly at hook time, `action_queue.cpp:412`, while the draw is queued later at `:526`) plays **A** and draws B–F. Different card played, different hand, and the second-order RNG differs too: at ≥ 2 stacks the Java spends **all** its target rolls before **any** play, so a play that consumes `card_random_rng` or kills a monster cannot influence the next target, whereas the engine interleaves roll/play/roll/play. **The landed test asserts the engine's version by name:** `CardColorlessRaresMayhem.TopCardIsPlayedBeforeTheTurnsDraw` (`tests/card_colorless_rares_test.cpp:1766`) puts a Defend on top of eight Strikes and requires the Defend to be PLAYED — in the game that Defend is DRAWN. That test and `TwoStacksPlayTwoCardsInDrawOrder` (`:1786`) both need re-basing with the fix. **Not fixed here** — this is engine code and this track is documentation-only; `registry/powers.yaml`'s Mayhem provenance and its section comment have been corrected in place to state the Java and to name this row. Shape of the fix (engine owner's call): queue a deciding item that rolls and then `add_to_bottom`s the `PLAY_CARD`, i.e. the same two-level shape `Opcode::RED_SKULL_ENTRY` already uses for the identical `addToBot`-inside-a-queued-action problem. **Distilled Chaos is NOT affected and needs no change** — there the `getRandomMonster` call is a constructor ARGUMENT and the `PlayTopCardAction` is queued directly, with no wrapper, which `potions.cpp:394-407` already models correctly and pins to capture evidence. **DISCHARGED by wave2-engine 596f49e (2026-07-28), RED-first, in exactly the shape this row proposed:** `kPlayCardDeferRoll` (PLAY_CARD flags bit 5, engine-only) — `op_play_card`'s defer branch rolls one live-monster target and re-queues the play, bit cleared and target baked, at the BOTTOM of the queue; `power_mayhem.cpp` queues one deferred item per stack, so the play lands behind the turn's draw and at ≥2 stacks every roll is spent before any play, matching the recovered `MayhemPower$1` two-level queue. The two named tests were re-based RED-first (`TopCardIsPlayedAfterTheTurnsDraw`, `TwoStacksPlayPostDrawInOrderRollingBeforeThePlays`) plus a new lethal-interrupt pin (`BothRollsSpentEvenWhenTheFirstPlayEndsCombat`); no new opcode was claimed and Distilled Chaos was confirmed untouched |
| **Eleven more +1000-class citations in `registry/relics.yaml`** (Turnip, ThreadAndNeedle, WingBoots, Shovel, StoneCalendar, UnceasingTop, MembershipCard, MedicalKit, PrismaticShard, Waffle, Toolbox) | wave2 integration sweep | **DISCHARGED — wave3-citations, 2026-07-28 (S3.65 re-verified 2026-09-03)** | Found while fixing Orrery and Cauldron on the union (both confirmed +1000-class and corrected with their methods re-read in full: Orrery.onEquip is `Orrery.java:27-33` in a 40-line file, Cauldron.onEquip `Cauldron.java:30-45` in a 52-line file). A `grep -oE "[A-Za-z]+\.java:1[0-9]{3}" registry/*.yaml` sweep then shows eleven MORE small-file relics citing 4-digit lines their files cannot contain — every one of the eleven files is 30-75 lines (re-measured in the decompiled tree) against citations at :1024-:1364. Same evidence signature as the nine-citations row: one bad authoring batch, NOT a mechanical offset, so each needs its cited method RE-READ IN FULL before renumbering — do not line-shift. None is in `cards.yaml`, so the sidetable regeneration trap does not apply. The `AbstractDungeon`/`AbstractPlayer` 4-digit citations in the same sweep are legitimate (those files really are that long) and are NOT part of this row. **DISCHARGED — wave3-citations, 2026-07-28 (provenance strings only; no mechanic, no schema, no generated artefact).** All eleven methods re-read IN FULL in the decompiled tree before renumbering; the row's "bad authoring batch, NOT a mechanical offset" premise is CONFIRMED — the eleven per-file deltas are all different (+991 Toolbox, +995 Shovel, +1047 StoneCalendar, +1118 MedicalKit, +1123 ThreadAndNeedle, +1181 MembershipCard, +1202 PrismaticShard, +1253 Turnip, +1272 Waffle, +1283 UnceasingTop, +1346 WingBoots), and WingBoots is not even self-consistent (+1346 on the constructor, +1345 on `setCounter`/`canSpawn`). Old→new: `Turnip.java:1265-1281`→`:12-29` (whole 30-line class) · `ThreadAndNeedle.java:1154-1159`→`:31-35` (42) · `WingBoots.java:1364-1367`→`:18-21`, `:1374-1381`→`:29-35`, `:1383-1386`→`:38-40` (47) · `Shovel.java:1024-1035`→`:33-43`, `:1037-1040`→`:46-48` (55) · `StoneCalendar.java:1083-1116`→`:36-68` (75) · `UnceasingTop.java:1331-1338`→`:48-54`, `:1321-1325`→`:38-41`, `:1316-1319`→`:33-35` (61) · `MembershipCard.java:1198-1215`→`:17-36` (43) · `MedicalKit.java:1153-1159`→`:35-41` (43) · `PrismaticShard.java:1236-1241`→`:34-38` (40) · `Waffle.java:1300-1303`→`:28-31` (38) · `Toolbox.java:1024-1028`→`:29-32` (39). Ranges follow the Orrery/Cauldron convention set by the deferring commit: signature line through closing brace, `@Override` excluded; whole-class span for marker relics (the StrangeSpoon shape). **Every prose verdict is MATCH** — no relic's description contradicts its method's behaviour, so nothing here is a stop-the-line. Four in-row corrections landed alongside: UnceasingTop's condition named the field `disabledUntilTurnEnds` (the field is `disabledUntilEndOfTurn`; `disableUntilTurnEnds()` is the setter); PrismaticShard's prose gave only the `chosenClass != DEFECT` clause and dropped the `&& masterMaxOrbs == 0` re-entry guard; Turnip's "no overrides" now reads "no behavioural hook overrides" (it does carry the universal `getUpdatedDescription`/`makeCopy` boilerplate); and the same row's cross-file `AbstractCard.java:914-924` → `:916-924` (it opened two lines early, on `canPlay`'s closing brace — `canUse` itself is 916-924). Also fixed in the same file because a mechanical sweep can NEVER find it: `ChemicalX.java:12-30`→`:13-34` — in range for its 35-line file, but it is StrangeSpoon's span copied onto the ChemicalX row (StrangeSpoon's own `:12-30` re-read and CONFIRMED correct). **THE ROW UNDERCOUNTED, TWICE — see the two new rows below**: its `1[0-9]{3}` grep could only ever see 4-digit citations beginning with 1, so it missed fifteen further out-of-range citations in this same file (three-digit and 2xxx drift, `Girya.java:519` and `PeacePipe.java:862` among them), and it was wrong that "none is in `cards.yaml`" — nine card rows are out of range too. Neither group was touched here; both are enumerated below with real line counts already measured |
| **Fifteen MORE out-of-range citations in `registry/relics.yaml`** (Ginger, OldCoin, IceCream, Calipers, PrayerWheel, Girya, DuVuDoll, Pocketwatch, Mango, IncenseBurner, PeacePipe, CaptainsWheel, FossilizedHelix, CharonsAshes, ChampionsBelt) | wave3-citations, 2026-07-28 | **DISCHARGED — wave3-citations, 2026-07-28 (S3.65 re-verified 2026-09-03)** | The eleven-citations row's own sweep used `grep -oE "[A-Za-z]+\.java:1[0-9]{3}"`, which can only match a 4-digit line beginning with `1` — so it was structurally blind to three-digit and 2xxx drift in the very file it was auditing. A full scan (every `Name.java:N` in `registry/*.yaml` against the real `wc -l` of the decompiled file) finds fifteen more, cited→real-file-length: `Ginger.java:473-489`+`:119-124` (30) · `OldCoin.java:812-816`+`:818-821` (46) · `IceCream.java:574-590` (30) · `Calipers.java:60-77`+`:353-359` (31) · `PrayerWheel.java:986-989` (37) · `Girya.java:519-559` (69) · `DuVuDoll.java:303-354` and its three sub-ranges (82) · `Pocketwatch.java:917-946` (67) · `Mango.java:771-774` (38) · `IncenseBurner.java:623-637` (51) · `PeacePipe.java:862-873`+`:875-878` (56) · `CaptainsWheel.java:110-133` (60) · `FossilizedHelix.java:392-398` (48) · `CharonsAshes.java:219-227` (50) · `ChampionsBelt.java:172-176`+`:111-113` (42). **Girya and PeacePipe are provably the same bad batch as the eleven**: their `canSpawn` bodies are byte-identical to Shovel's, and Shovel's was just corrected to `Shovel.java:33-43` while these still read `:519-559` and `:862-873`. Two traps for the owner. (1) **Some rows are MIXED** — IceCream carries `:574-590` *and* `:25-40`/`:59-68`, so a row is not uniformly wrong and a per-row offset is meaningless; re-read every method, do not line-shift. (2) **A bare `:N-M` continuation citation is invisible to the scan above**, which only matches `Name.java:N` — IceCream's `:59-68` is out of range for a 30-line file and was found by eye, so the mechanical list is a FLOOR, not the full set. Also note `ChemicalX.java:12-30`, fixed with the eleven: it was *in range* for its 35-line file and no out-of-range sweep could ever have caught it. **DISCHARGED — wave3-citations, 2026-07-28 (provenance strings only).** All fifteen re-read in full. Old→new: `Ginger.java:473-489`→`:12-29` (30, whole class) · `OldCoin.java:812-816`→`:31-34`, `:818-821`→`:37-39` (46) · `IceCream.java:574-590`→`:12-29` (30, whole class) · `Calipers.java:60-77`→`:12-30` (31, whole class) · `PrayerWheel.java:986-989`→`:33-35` (37) · `Girya.java:519-559`→`:27-62` (69) · `DuVuDoll.java:303-354`→`:43-75`, `:321-332`→`:43-53`, `:334-345`→`:56-66`, `:347-354`→`:69-75` (82) · `Pocketwatch.java:917-946`→`:32-60` (67) · `Mango.java:771-774`→`:29-31` (38) · `IncenseBurner.java:623-637`→`:31-44` (51) · `PeacePipe.java:862-873`→`:34-44`, `:875-878`→`:47-49` (56) · `CaptainsWheel.java:110-133`→`:31-53` (60) · `FossilizedHelix.java:392-398`→`:31-36` (48) · `CharonsAshes.java:219-227`→`:36-43` (50) · `ChampionsBelt.java:172-176`→`:32-35` (42). **Girya/PeacePipe/Shovel `canSpawn` re-confirmed byte-identical line by line** (`Girya.java:47-57`, `PeacePipe.java:34-44`, `Shovel.java:33-43`). Prose verdict **MATCH on all fifteen**. Cross-file citations in the same rows: `ApplyPowerAction.java:119-124` (Ginger), `:111-113` (ChampionsBelt) and `GameActionManager.java:353-359` (Calipers) re-read and **CORRECT as cited**; three were not — `EnergyManager.java:25-40`→`:25-41` and `EnergyPanel.java:59-68`→`:59-69` (both stopped one line short of the closing brace), and Mango's open-ended `AbstractCreature.java:199+`→`:199-209`. A sixteenth was found by eye and fixed: **`FaceOfCleric.java:22-26`→`:27-30`** — it pointed at `getUpdatedDescription`, and like ChemicalX it was IN RANGE, so no sweep can find its kind. **CORRECTION TO THIS ROW AS ORIGINALLY FILED:** the "some rows are MIXED — IceCream carries `:574-590` *and* `:25-40`/`:59-68`" claim was **wrong**, and it was wrong because it was read off a `grep -oE` alternation that had stripped the filenames. Those two are `EnergyManager.java:25-40` and `EnergyPanel.java:59-68` — different files, properly qualified, and merely off by one rather than drifted. No relics.yaml row mixes drifted and correct citations *to its own file*. The **bare `:N-M` continuation** trap is real and stands, but its instances are OldCoin, DuVuDoll and PeacePipe (and, in the eleven, WingBoots/Shovel/UnceasingTop) — not IceCream |
| **`increaseMaxHp`'s second parameter does not do what three registry rows say it does** (Waffle, Mango, FaceOfCleric; Pear is clean) | wave3-citations, 2026-07-28 | **DISCHARGED — wave3-integrate, 2026-07-29** | **A prose-vs-behaviour mismatch, surfaced and flagged in place rather than rewritten away.** `AbstractCreature.increaseMaxHp(int amount, boolean showEffect)` (`AbstractCreature.java:199-209`) is the **only** definition of the method in the decompiled tree — `AbstractPlayer` does not override it — and **its body never reads `showEffect`**. The `this.heal(amount, true)` at `:206` is unconditional, as is the `TextAboveCreatureEffect` at `:205`. Three rows assert otherwise: `registry/relics.yaml` **Waffle** ("The `false` means increaseMaxHp does NOT heal the gained amount" — false; `increaseMaxHp(7, false)` heals 7), **Mango** ("heals the gained amount *when the flag is set*" — outcome right, mechanism wrong), **FaceOfCleric** ("`AbstractPlayer.increaseMaxHp(amount, heal)` … *with heal set*" — wrong class and wrong parameter name). **Pear** (`Pear.java:29-31`, verified correct) says only "increaseMaxHp heals the gained amount" and is clean. **No observable divergence today**: Waffle's sim body (`src/engine/relics/relic_pickup_shop.cpp`) does `max_hp += 7; hp = max_hp;`, and the Java's heal-7-then-heal-to-full reaches the same state. The exposure is forward-looking and two-sided — Waffle's comment explicitly justifies keeping two separate steps so "the shape survives a future partial-heal modifier", which is **backwards** (under such a modifier the Java's `increaseMaxHp` heal would itself be scaled, while the sim does one raw assignment), and anyone who trusts the `false` and deletes the heal-to-full would silently break the relic. All three yaml claims are **preserved verbatim with a `MISMATCH FLAGGED` annotation appended** — deliberately not rewritten, per the standing rule that a prose/behaviour contradiction is reported, not smoothed over. The owner must also fix the mirror comment in `src/engine/relics/relic_pickup_shop.cpp:10-27`, which repeats the false claim and is outside a citation pass's scope **DISCHARGED — wave3-integrate, 2026-07-29 (prose + comment only; no mechanic, no schema, no generated artefact).** The finding was re-verified against the decompiled tree before anything was rewritten: `AbstractCreature.increaseMaxHp(int amount, boolean showEffect)` at `:199-209` is still the only definition (`grep` over the whole tree returns exactly that one signature — `AbstractPlayer` does not override it), the body contains no reference to `showEffect` at all, and `this.heal(amount, true)` at `:206` is unconditional. All three yaml claims are now REWRITTEN to the verified mechanism rather than left flagged — the `MISMATCH FLAGGED` scaffolding is gone from `registry/relics.yaml` (0 occurrences remain) and each row carries a dated `MECHANISM CORRECTED` note naming `:199-209`/`:206`: **Waffle** now states that the `false` suppresses nothing and that `increaseMaxHp(7, false)` raises max HP by 7 AND heals 7, **Mango** drops "when the flag is set", **FaceOfCleric** drops the non-existent `AbstractPlayer.increaseMaxHp(amount, heal)`. The inverted rationale is retired explicitly in each note (under a partial-heal modifier the Java's own `increaseMaxHp` heal would be scaled too, so the old argument for keeping two steps ran backwards). The mirror comment in `src/engine/relics/relic_pickup_shop.cpp` is rewritten in the same pass and now carries the load-bearing instruction the row asked for: **do not delete `hp = rs.max_hp` on the strength of the `false`** — the flag is inert, both Java steps heal, and dropping the assignment would leave Waffle granting max HP without the HP. `MagicFlower.java:32` in that comment was re-read and is CORRECT as cited (the COMBAT phase gate). Registry regenerated via `gen.py`: generated headers byte-identical to the pre-edit baseline, confirming provenance-only. |
| **Nine out-of-range citations in `registry/cards.yaml`** (BodySlam, Cleave, Clothesline, Headbutt, IronWave, SwordBoomerang, ThunderClap, TwinStrike, WildStrike) | wave3-citations, 2026-07-28 | **DISCHARGED — wave3-citations, 2026-07-28 (S3.65 re-verified 2026-09-03)** | The eleven-citations row asserted "None is in `cards.yaml`, so the sidetable regeneration trap does not apply". **That is false**, and the trap therefore DOES apply to this row: cited→real-file-length `BodySlam.java:95-98`/`:127-131` (80) · `Cleave.java:243-244`/`:248-252`/`:255-260` (56) · `Clothesline.java:300-301`/`:305-308`/`:311-317` (56) · `Headbutt.java:356`/`:360-363`/`:366-371` (53) · `IronWave.java:499-500`/`:504-510`/`:513-520` (62) · `SwordBoomerang.java:658-662` (52) · `ThunderClap.java:718-728` (65) · `TwinStrike.java:780-782` (54) · `WildStrike.java:838-841` (57). Whoever takes this must regenerate `tools/oracle_bridge/driver/cards_sidetable.json` via its `_provenance.regenerate` command in the same commit and expect a hash-line-only diff. Same bar as above: re-read each cited method in full, do not line-shift — and note the shape here differs from the relics batch (each card row carries three citations that drift *together*, which looks like a per-file offset and is exactly the thing that must not be assumed). **DISCHARGED — wave3-citations, 2026-07-28.** All nine `use`/`upgrade` bodies and every cited field line re-read in full. Old→new (use · fields · upgrade): `BodySlam` `:95-98`→`:36-42` · — · `:127-131`→`:68-73` (80) · `Cleave` `:248-252`→`:37-41` · `:243-244`→`:32-33` · `:255-260`→`:44-49` (56) · `Clothesline` `:305-308`→`:37-40` · `:300-301`→`:32-33` · `:311-317`→`:43-49` (56) · `Headbutt` `:360-363`→`:35-38` · `:356`→`:31` · `:366-371`→`:41-46` (53) · `IronWave` `:504-510`→`:39-46` · `:499-500`→`:34-35` · `:513-520`→`:49-55` (62) · `SwordBoomerang` `:658-662`→`:33-37` · `:653-654`→`:28-29` · `:664-670`→`:40-45` (52) · `ThunderClap` `:718-728`→`:40-50` · `:713-714`→`:35-36` · `:730-736`→`:53-58` (65) · `TwinStrike` `:780-782`→`:36-39` · `:775`→`:31`, `:776`→`:32` · `:785-791`→`:42-47` (54) · `WildStrike` `:838-841`→`:39-42` · `:832`→`:33`, `:833`→`:34` · `:843-849`→`:45-50` (57). Prose verdict **MATCH on all nine**. **The row's warning about the shape was right, and the measurement now says why not to trust it:** the per-file deltas ARE internally consistent (+59 BodySlam, +211 Cleave, +268 Clothesline, +325 Headbutt, +465 IronWave, +625 SwordBoomerang, +678 ThunderClap, +744 TwinStrike, +799 WildStrike) but every file's is different and each `upgrade()` range is one further off because the old ranges included the `@Override` line the current convention excludes — so a per-file shift would have silently mis-set nine ranges by one. The monotonically increasing offsets look like citations taken against a **concatenated dump** of the card sources rather than the individual files; that is a hypothesis, not a licence to arithmetic. Two prose tightenings added while reading, neither a contradiction: BodySlam's `applyPowers` (`:45-51`) recomputes `baseDamage` from `currentBlock` exactly as `use` does (which is what makes the previewed number track block), and ThunderClap's Vulnerable loop (`:47-49`) has **no** `isDeadOrEscaped` guard where the cosmetic lightning-VFX loop above it (`:42-45`) does. `tools/oracle_bridge/driver/cards_sidetable.json` regenerated via its `_provenance.regenerate` command in the same commit; the diff is the single `source_sha256` line, as predicted, and `oracle_campaign_python_test` / `CardSideTableTest` are green. **A full scan now reports zero unresolvable `Name.java:N` citations across all of `registry/*.yaml`** — but that scan cannot see bare `:N-M` continuations or in-range-but-wrong citations (the ChemicalX and FaceOfCleric shapes), so it is a floor, not a proof |
| **~60 out-of-yaml MIRROR sites still carry the citations this branch corrected in `registry/*.yaml`** | wave3-citations, 2026-07-28 | **DISCHARGED — wave3-integrate, 2026-07-29 (67 replacements across 19 files); S3.65 (2026-09-03) re-swept the whole tree with a committed checker and found no surviving instance of any old citation** | Fixing the registry created a deliberate, recorded split-brain: `src/`, `include/`, `tests/` and `tools/` repeat the same drifted numbers. **Every mapping below was re-derived from the decompiled tree this wave — full method re-reads, and each single-line target was printed and semantically confirmed, not shifted.** Method spans use the Orrery/Cauldron convention (signature line → closing brace, `@Override` excluded). Old→new by Java file: `StoneCalendar` `:1083-1116`→`:36-68`, `:1101`→`:53` (the `addToBot DamageAllEnemiesAction`), `:1112-1116`→`:65-68` · `CaptainsWheel` `:110-133`→`:31-53`, `:123`→`:43` (`addToBot GainBlockAction 18`), `:129-133`→`:50-53` · `Pocketwatch` `:917-946`→`:32-60`, `:906-946`→`:20-60` (`:20` is the `firstTurn` field), `:923`→`:39-43` (the `counter <= 3 && !firstTurn` branch), `:926`→`:40` (`addToBot DrawCardAction 3`), `:942-946`→`:57-60` · `IncenseBurner` `:623-637`→`:31-44`, `:628-637`→`:36-44`, `:630`→`:37` (the `-1` save-loaded ternary), `:635`→`:42` (`addToBot ApplyPowerAction Intangible`) · `DuVuDoll` `:303-354`→`:43-75`, `:321-332`→`:43-53`, `:334-345`→`:56-66`, `:321-345`→`:43-66`, `:347-354`→`:69-75` · `Girya` `:519-559`→`:27-62`, `:529-536`→`:38-44`, `:538-549`→`:47-57`, `:532`→`:41` (`addToTop ApplyPowerAction Strength`) · `PeacePipe` `:862-873`→`:34-44`, `:875-878`→`:47-49` · `Shovel` `:1024-1035`→`:33-43`, `:1037-1040`→`:46-48` · `WingBoots` `:1364-1367`→`:18-21`, `:1383-1386`→`:38-40` · `ThreadAndNeedle` `:1154-1159`→`:31-35` · `MedicalKit` `:1153-1159`→`:35-41` · `Waffle` `:1300-1303`→`:28-31` · `Mango` `:771-774`→`:29-31` · `OldCoin` `:812-816`→`:31-34`, `:818-821`→`:37-39` · `PrayerWheel` `:986-989`→`:33-35` · `FossilizedHelix` `:392-398`→`:31-36` · `CharonsAshes` `:219-227`→`:36-43` · `ChampionsBelt` `:172-176`→`:32-35` · `MembershipCard` `:1-56`→`:17-36` · `BodySlam` `:96`→`:37` · `WildStrike` `:840`→`:41` · `Orrery` `:1270-1276`→`:27-33` and `Cauldron` `:1075-1092`→`:30-45` (wave-2 fixed the yaml and missed this mirror). Sites: `src/engine/relics/relics_rare.cpp:57,77,81,88,106,117,129,148,154,192,200,209,229,233` · `relics_shop.cpp:75` · `relic_pickup_rare.cpp:13,17,23,32,33,70,88,96` · `relic_pickup_shop.cpp:18` · `src/engine/relic_pools.cpp:169` · `src/engine/action_queue.cpp:392` · `src/engine/interp/interp_powers.cpp:227` · `include/sts/engine/interp.hpp:150` · `relic_pools.hpp:52` · `run_deck.hpp:121` · `rest_sites.hpp:20,22,24` · `shop.hpp:79` · `tests/relic_rares_shop_test.cpp:210,248,276,292,310,327,355,400,427,857,909,958,1104,1288,1289,1290,1314` · `tests/run_advance_test.cpp:1022,1061,1063,1087,1101,1107` · `tests/combat_start_test.cpp:418` · `tests/card_attacks_test.cpp:5,13` · `tools/registry_gen/stsgen/vocab.py:42` · `tools/oracle_bridge/replay/src/command_map.hpp:297,298` · `tools/oracle_bridge/driver/g6_campaign2_spotdiff.md:220`. **THREE separate sub-families, do not treat them as one sweep.** (a) The drifted majority, above. (b) **A near-miss `addCampfireOption` family at `include/sts/engine/rest_sites.hpp:20,22,24`** that is NOT the same defect and is off by only a few lines: `Girya.java:50-53`→`:60-62`, `PeacePipe.java:47-50`→`:47-49`, `Shovel.java:47-50`→`:46-48`. (c) **IN-RANGE-but-wrong citations no sweep can find** — `src/engine/action_queue.cpp:392` cites `DuVuDoll.java:31` as "an addToTop atBattleStart body"; `:31` is a bare `}` closing `getUpdatedDescription`, and the addToTop is `:72`. **VERIFIED CORRECT, do not touch:** `PeacePipe.java:48` (the `TokeOption` gate) at `src/engine/rest_sites.cpp:35,164`, `run_advance.cpp:2062`, `relic_pools.hpp:203`, `tests/rest_sites_test.cpp:690,735`; and `ApplyPowerAction.java:111-113` at `interp_powers.cpp:227` / `relic_rares_shop_test.cpp:857`. **`docs/stage-b-log.md:5212-5213` is deliberately NOT in this handoff**: it carries a third spelling again (`SmilingMask.java:131-162`, `Courier.java:181-212`, `MembershipCard.java:229-255`, every one out of range — those files are 50, 50 and 43 lines) but it is an append-only archive of what a past run recorded, which §8 treats as history — the correction is recorded here instead of edited there, per the established precedent. `docs/stage-b-log.md:963-977` (`BodySlam.java:96`, `HeavyBlade.java:426`, `PerfectedStrike.java:565`, `Clash.java:184`) is out of range for the same reason and gets the same treatment. **Also unverified and outside this pass:** the `shop.hpp:79` block's `SmilingMask.java:1-60` and `Courier.java:1-60` are both wrong (each file is 50 lines); the real bodies are `onEnterRoom :31-38` and `canSpawn :41-43` in BOTH files, which are byte-identical to each other and, for `canSpawn`, to `OldCoin.java:37-39` **APPLIED — wave3-integrate, 2026-07-29 (comments/provenance only; no mechanic, no schema, no generated artefact).** Every mapping was re-confirmed against the decompiled tree before application, not taken on trust: all 23 cited files match the line counts this row recorded, and each single-line target was printed and its content checked against the description in this row (`StoneCalendar:53` = the `addToBot DamageAllEnemiesAction`; `CaptainsWheel:43` = `addToBot GainBlockAction 18`; `Pocketwatch:20` = the `firstTurn` field, `:40` = `addToBot DrawCardAction 3`, `:39-43` = the `counter <= 3 && !firstTurn` branch; `IncenseBurner:37` = the `-1` ternary, `:42` = the Intangible apply; `Girya:41` = the `addToTop` Strength; and so on). **The line numbers in this row had already moved** — the followup merge shifted `run_advance_test.cpp:1022`→`1173`, `rest_sites_test.cpp:690`→`705`, `run_advance.cpp:2062`→`2144`, `command_map.hpp:297-298`→`298-299`, `shop.hpp:79`→`77-79` — so every site was re-located BY CONTENT, exactly as the row instructed. **67 replacements across 19 files**, applied in a single non-cascading pass (maximal-token match against a fixed old→new table, so no partial match and no rewrite of a rewrite). **The three sub-families were kept apart:** (a) the drifted majority; (b) the near-miss `addCampfireOption` trio in `rest_sites.hpp` (`Girya.java:50-53`→`:60-62`, `PeacePipe.java:47-50`→`:47-49`, `Shovel.java:47-50`→`:46-48`) — verified as the `addCampfireOption` bodies, distinct from the `canSpawn` renumbering in the same files; (c) the in-range-but-wrong `action_queue.cpp` citation, fixed by hand to `DuVuDoll.java:72` after confirming `:31` really is the bare `}` closing `getUpdatedDescription` and `:72` really is the `addToTop ApplyPowerAction` Strength inside `atBattleStart` (`:69-75`). **The do-not-touch list was honoured and re-checked afterwards:** all six `PeacePipe.java:48` sites and both `ApplyPowerAction.java:111-113` sites are byte-unchanged — `interp_powers.cpp` and `relic_rares_shop_test.cpp` each carry a correct `ApplyPowerAction` citation on the same line as a corrected `ChampionsBelt` one, and only the latter moved. **Five mappings went unused and are recorded as skipped, not missed:** `DuVuDoll.java:303-354`, `Girya.java:519-559`, `PeacePipe.java:862-873`, `PeacePipe.java:875-878`, `Shovel.java:1037-1040` have ZERO out-of-yaml occurrences repo-wide (they were yaml-only citations the citations branch had already fixed). The `shop.hpp` block needed prose, not renumbering: `SmilingMask.java:1-60` and `Courier.java:1-60` are replaced by the real bodies `onEnterRoom :31-38` / `canSpawn :41-43` in each, with the byte-identity to one another and of `canSpawn` to `OldCoin.java:37-39` recorded in the block. `docs/stage-b-log.md` left unedited as this row directs. Split-brain CLOSED: a repo-wide re-scan finds no surviving instance of any old citation in this table. |
| CFR holes recovered from the shipped jar — the tree is no longer lossy for the 16 | `redskull_capture_runbook.md` §6 | **DISCHARGED 2026-07-28 (Track P, wave-2)** | The runbook's recommendation was carried out. **Counts re-derived, runbook CONFIRMED exactly:** 145 files / 429 marker sites tree-wide, of which **16 files / 37 sites** are `com/megacrit` — the per-file marker counts sum to 37 and the game jar holds exactly 37 anonymous inner classes for those 16 outers. **49 files were recovered** into `D:\STS_BG_Mod\SlayTheSpireDecompiled` as NEW siblings (`RedSkull$1.java` beside `RedSkull.java`), each with a full provenance header, indexed by a new `RECOVERED-INNER-CLASSES.md` at the tree root: 37 anonymous + 6 switch-map + 5 nested types for the 16, plus `AbstractDungeon$1`. Nothing existing was modified, moved or reflowed, so **every citation in this repo resolves exactly as before**. All 17 outer classes verified **byte-identical** between `sts-classes.jar` and `desktop-1.0.jar` (17/17, 0 differ); `RedSkull.class` reproduces the runbook's `ea4d35bd…` and both jar SHA-256s match §2. **CORRECTION to §6's framing — the marker count understates the damage.** The marker fires only for ANONYMOUS classes; javac's **switch-map** classes were stripped too and their loss is SILENT — CFR emits `switch (1.$SwitchMap$…[x.ordinal()])` with bare integer case labels and no warning. Tree-wide **129 `com/megacrit` files reference a `$SwitchMap` and 123 of them carry no anonymous marker at all**, i.e. 123 megacrit files are damaged in a way nothing announces. `AbstractDungeon.java` alone has 12 such switches; `AbstractDungeon$1` was recovered for that reason and decodes them (`CardColor` COLORLESS=1 CURSE=2; `CardRarity` BASIC=1 COMMON=2 UNCOMMON=3 RARE=4 CURSE=5; `RelicTier` COMMON=1 UNCOMMON=2 RARE=3 SHOP=4 BOSS=5), confirming the engine's reading of `transformCard` (`:861`) and `returnTrulyRandomCardFromAvailable` (`:1018`). **The other ~122 are NOT recovered** — out of this task's scope; same two-command recipe applies, and re-extracting `sts-classes.jar` with inner classes retained remains the structural fix. **AUDIT of every recovered body against landed sim behaviour: ONE contradiction (Mayhem, its own row above), everything else CLEAN and two long-standing inferences now upgraded to evidence.** (1) `Looter$1`/`$2` (`:97`/`:115`) and `Mugger$1`/`$2` (`:95`/`:113`) read exactly `stolenGold += Math.min(goldAmt, player.gold)`, queued ahead of the `DamageAction` — `monsters.yaml`'s inference from the synthetic accessors was right, and is now annotated as confirmed. (2) `AbstractRoom$1` (`:409`) matches the javap reconstruction in `action_queue.cpp:186-200` line for line. (3) `RedSkull$1` reproduces the runbook §3 body byte for byte, and the `final-integrate` `RED_SKULL_ENTRY` fix stands. (4) `AbstractRoom$2` confirms `COMBAT = 2`, so the turn-1 block really is `AbstractRoom.java:236-258`. **ONE DOC HANDOFF (engine-owned, not a defect):** `include/sts/engine/action_queue.hpp` note (2) reasons from *"the decompiled `GameActionManager.java` never sets `monsterAttacksQueued` back to false anywhere in the tree … so this decompiled copy is missing the reset"* — the reset was never missing, it was inside the stripped `AbstractRoom$1`, which ends `AbstractDungeon.actionManager.monsterAttacksQueued = false`. The note's **conclusion is correct** (clear it at the end-turn sentinel) and no code changes; the premise should be rewritten to cite the recovered class instead of inferring from an absence. Header file, so handed to the engine owner rather than edited here — **CONSUMED by wave2-engine 9561c8e (addendum 3):** the note now cites the recovered `AbstractRoom$1` reset instead of inferring from an absence; conclusion and code unchanged |
| Egg trio onEquip reward-screen preview pass | wave2-engine stage 3d (stale-comment re-derivation) | **DISCHARGED — wave3-followup (2026-07-28)** | **A "does not exist yet" comment whose prerequisite arrived, hiding a REAL (narrow) gap.** `FrozenEgg2.onEquip` (FrozenEgg2.java:31-38; Molten/Toxic identical) walks the OPEN combat-reward screen's card offers and upgrades matching-type cards in place (`onPreviewObtainCard` → `onObtainCard`). The engine's three egg `on_equip` bodies are no-ops whose old justification — "the reward screen does not exist yet" — expired: `RewardScreen`/`RunRewardItem.card_upgrades` exist, and an elite reward screen holds a RELIC and a CARDS item at once, so claiming an egg with an offer still open is reachable in S1. **Scope of the divergence**: the still-open OFFER's upgrade bits only (a reward-screen observation the run differ compares); the MASTER DECK converges regardless — the pick lands through `onObtainCard` with the egg then owned, and the Java's `!upgraded` gate makes preview-then-obtain and obtain-only end identical. **Why not fixed in the sweep**: the plain `on_equip` dispatch receives `(RunState, miscRng, slot)` — the reward screen travels only in `RelicEquipContext` (`on_equip_screen` bodies), and the eggs cannot move to that surface because the ctx-less `acquire_relic` door (event grants in `exordium_events_*`, the shop fallback) refuses `on_equip_screen` ids by design (`NEEDS_EQUIP_CONTEXT`). The fix needs a deliberate decision about how every equip site sees the screen (e.g. an optional `RewardScreen*` on the plain surface, or threading ctx everywhere) — an API change across all acquisition sites, materially larger than the stale-comment sweep that found it. Sites annotated: `relic_pickup_uncommon.cpp` (all three bodies), Frozen Egg's yaml provenance. **RESOLUTION (wave3-followup): the equip-plumbing decision is NEITHER candidate — no surface changes at all.** The pass lives at the CLAIM SITE (`claim_reward`'s RELIC arm, `apply_egg_preview_to_open_offers` in combat_rewards.cpp), the one place an open `RewardScreen` and an equip meet; it is EGG-GATED (`relic_equip_previews_open_offers` — Ceramic Fish/Darkstone Periapt have onObtainCard bodies but no Java onEquip walk, pinned by the `RewardEggPreview.ANonPreviewingOnObtainRelicLeavesTheOffersAlone` control) and REPLAYS the guarded onObtainCard body over a scratch row so preview and obtain cannot drift. The ctx-less acquire_relic door still refuses on_equip_screen ids untouched; an egg granted screenless (events, shop fallback) walks the game's stale/empty screen — observably inert — so every other path faithfully does nothing. Narrow observable pinned: `RewardEggPreview.ClaimingAnEggUpgradesTheMatchingStillOpenOffers` + `PreviewThenObtainDoesNotDoubleUpgrade` (the !upgraded gate). The three engine comment sites are updated; Frozen Egg's yaml provenance annotation is the provenance owner's (registry provenance strings are under the concurrent citations task). **THE ROW'S LAST THREE CONSUMERS ARE NOW LANDED TOO — `equip-trio`, 2026-08-27, and the row is closed for good.** The "equip-plumbing decision" this row recorded was cited by three further deferred bodies — Orrery, Dolly's Mirror and Cauldron (`registry/relics.yaml` rows 99 / 100 / 107), the SHOP relics whose `onEquip` opens a screen at the purchase site — and the resolution held a third time: **no dispatch surface changed.** All three went onto the EXISTING `pickup: on_equip_screen` surface with their bodies in `relic_pickup_shop.cpp`, and the purchase site translates their requests onto controller state that already existed: Orrery / Cauldron ask for `ITEM_REWARD` and `apply_claim_equip_request` presents `rc.rewards` as `RunPhase::COMBAT_REWARD` over the merchant (which is literally what `combatRewardScreen.open` does inside the still-mounted `ShopRoom`, and `ProceedButton.java:122-158` is why the shop is unreachable afterwards — the `open_dig_reward` precedent, one room over); Dolly's Mirror asks for the one genuinely new request, `GRID_DUPLICATE`, carried by a `pending_deck_pick` overlay that is `pending_bottle`'s sibling and cost **zero bytes** — it took one byte of the declared `pad2`, so no `RunController` hash moved and no fixture regenerated. Two corrections the bodies' authoring found, neither of them in the capture note this row inherited: **Orrery offers FIVE card rows, not four** (`setupItemReward` appends its own unconditional row on top of the four `addCardToRewards` calls, and unlike Cauldron nothing deletes it), and **a reward roll made in the merchant's room uses the SHOP's rarity table** — `ShopRoom` raises `baseRareCardChance` to 9 and forwards `useAlternation = false` (`ShopRoom.java:35-36`, `:52-55`), so both bodies roll 9/37 with N'loth's Gift inert. Evidence: seed **STS430130**, the S2.43 residual whose PART was blocked on exactly this, now replays **CLEAN to its run terminal, 88 records compared, zero divergences**, and the 94-artifact resweep moved 75 CLEAN / 19 PART → **76 CLEAN / 18 PART** with the PART set now exactly the 18 recaptured escape-window artifacts. The replay harness's `relic_on_equip_deferred` list is consequently **EMPTY** (kept, with its two callers, for the next deferred body) |
...
| fuzz `MoveCat` (`tools/fuzz/include/sts/fuzz/policy.hpp`) | **0–27** (14–20 rest-site B4.9, 21–22 treasure B4.7, 23 `EVENT_OPTION` B4.10, 24 `EVENT_GRID` B4.11, 25 `CHOICE_CONFIRM` B3.10c, 26 `SHOP` B4.8, 27 `RECALL` wave3-followup (the Ruby Key campfire button) — **spent**; `COUNT = 28`) | 28+ free — claim here first and bump `COUNT` past every enumerator (Wave-C track 2's 27–29 allocation was RELEASED unspent; see the Wave-C table) |
...
| Namespace | Grant |
|---|---|
| `MonsterId` | S2.23 **37–39**; S2.24 **40–44**; S2.25 **49–53**; S2.26 **54–57**; S2.27 **58–61**; S2.28 **62–65**; S2.32 **45–47** (BanditPointy game_id "BanditChild", BanditLeader, BanditBear — claimed 2026-08-09 at the S2.3x wave, out of the range this table had left unissued) (48, 66 unissued) |
...
- **Wave-3: the follow-up + citations union proven on `wave3-integrate`** `[x]`
  ...
  (4, `d3fd1c6`) onto master `5b91864`; merge commit `0a392db`, plus one
  integration commit applying the consolidated out-of-yaml handoff. NOT yet on
  `master` — landing is the orchestrator's. **followup — 6/6 follow-ups from
  the wave-2 queue fixed, and two of them shared a root cause.** The two
  wave-2 (a)-candidates STS01789 and STS00353 were ONE engine defect, not two:
  `MonsterHelper`'s `bottomGet*` helpers (`MonsterHelper.java:799-822`)
  CONSTRUCT every candidate before `random(0, n-1)` keeps one, and each ctor
  has already drawn its max HP off `monsterHpRng` — so the sim rolled 2 HP
  draws where the game rolled 7 and every KEPT monster's HP came from the
  wrong stream position on the two over-constructing S1 programs. Fixed by
  carrying the construction trace (`constructed[]`/`kept_mask`) and burning the
  discarded candidates' ctor rolls. The **stacked Elixir defect** was likewise
  two faults in one class: the harness's HAND_SELECT `proceed` sent
  `CHOOSE(0xFF)` — read as an illegal slot toggle, a silent no-op that left
  Elixir's exhaust screen open and froze the sim mid-fight — now
  `ActionVerb::CONFIRM`; and Toy Ornithopter's `+5` was healed inline where
  `ToyOrnithopter.onUsePotion` is an `addToBot(HealAction)` queued BEHIND the
  potion's own actions. Also landed: `RecallOption` (Ruby Key) modelled, so the
  `Recall` rest stops are gone and MoveCat 27 is claimed (COUNT = 28);
  `RunState.boss_ids` given its first engine writer, so `act_boss` is compared
  for real and the differ's neutralization is REMOVED; the Egg-trio onEquip
  reward-screen preview pass resolved at the CLAIM SITE rather than by changing
  any dispatch surface; and the Smoke-Bomb escape-settlement race classified as
  a narrow field-set + window RACE (exactly one such record across all 49
  artifacts). **citations — the registry citation floor was reached.** All four
  batches discharged with every cited method RE-READ IN FULL, never
  line-shifted: the eleven +1000-class rows, fifteen more in `relics.yaml`,
  nine in `cards.yaml`, and the consolidated handoff. The "one bad authoring
  batch, not a mechanical offset" premise held every time — the eleven per-file
  deltas are all different (+991..+1346, WingBoots not even self-consistent),
  and the nine card rows' deltas, though internally consistent per file
  (+59..+799), differ from each other AND every `upgrade()` range was one
  further off because the old ranges included the `@Override` line the current
  convention excludes, so a per-file shift would have mis-set nine ranges by
  one. A full scan now reports ZERO unresolvable `Name.java:N` citations across
  `registry/*.yaml` — **a floor, not a proof**: it cannot see bare `:N-M`
  continuations nor in-range-but-wrong citations, two of which (ChemicalX,
  FaceOfCleric) were found by eye this wave. **The `increaseMaxHp`
  documentation defect** is the wave's stop-the-line, and it is now closed:
  `AbstractCreature.increaseMaxHp(int amount, boolean showEffect)`
  (`AbstractCreature.java:199-209`) is the ONLY definition in the tree,
  `AbstractPlayer` does not override it, and the body NEVER READS `showEffect`
  — `this.heal(amount, true)` at `:206` is unconditional. Three registry rows
  said otherwise (Waffle "the `false` means increaseMaxHp does NOT heal";
  Mango "when the flag is set"; FaceOfCleric naming a non-existent
  `AbstractPlayer.increaseMaxHp(amount, heal)`); the citations branch preserved
  all three verbatim under `MISMATCH FLAGGED` per its reporting rule, and this
  integration REWROTE them to the verified mechanism (0 flags remain) plus the
  mirror comment in `relic_pickup_shop.cpp`, which now carries the load-bearing
  instruction: **do not delete `hp = rs.max_hp` on the strength of the
  `false`** — the flag is inert, both Java steps heal, and dropping it would
  leave Waffle granting max HP without the HP. No observable divergence existed
  or exists; the exposure was entirely forward-looking. **The applied handoff**
  (the integration commit): the ~60-site out-of-yaml split-brain the citations
  branch deliberately created and recorded is CLOSED — **67 replacements across
  19 files**, every mapping re-confirmed against the decompiled tree first (all
  23 files' line counts match the row, every single-line target printed and
  checked against the row's description), every site re-located BY CONTENT
  because the followup merge had already moved the row's line numbers
  (`run_advance_test.cpp:1022`→`1173`, `rest_sites_test.cpp:690`→`705`,
  `run_advance.cpp:2062`→`2144`, `command_map.hpp:297-298`→`298-299`,
  `shop.hpp:79`→`77-79`). The row's THREE sub-families were kept apart — the
  drifted majority; the near-miss `addCampfireOption` trio in `rest_sites.hpp`;
  and the in-range-but-wrong `action_queue.cpp` citation, where `DuVuDoll.java:31`
  was confirmed to be a bare `}` and the `addToTop` confirmed at `:72` — and
  the VERIFIED-CORRECT list was honoured and re-checked afterwards: all six
  `PeacePipe.java:48` sites and both `ApplyPowerAction.java:111-113` sites are
  byte-unchanged, including the two lines that carry a correct
  `ApplyPowerAction` citation beside a corrected `ChampionsBelt` one. Five
  mappings went unused and are recorded as SKIPPED, not missed (zero
  out-of-yaml occurrences: they were yaml-only). `shop.hpp` needed prose, not
  renumbering (`SmilingMask`/`Courier` `:1-60` → the real `onEnterRoom :31-38`
  / `canSpawn :41-43`, byte-identical to each other and, for `canSpawn`, to
  `OldCoin.java:37-39`); `command_map.hpp`'s Orrery/Cauldron mirror — the
  wave-2 yaml fix's missed twin — is corrected; and Frozen Egg's yaml
  provenance now carries the supersession note the followup branch deferred to
  the provenance owner (its stale "STILL an explicit no-op" claim and its
  residual-divergence sentence are retired; the row it named is DISCHARGED).
  `docs/stage-b-log.md` left unedited per the append-only precedent.
  **Verification:** WSL `debug`/`asan`/`release` AND `win-debug`/`win-asan` —
  full suite PASS, 0 failed, all five (`win-*` clean-reconfigured after a host
  restart); `fuzz_soak --seeds 300` failures: 0; `check_stale_counts` and
  `check_doc_links` clean. Registry regenerated via `gen.py` and the generated
  headers are BYTE-IDENTICAL to the pre-edit baseline, which is the mechanical
  proof this integration was provenance/comment-only; `cards.yaml` is untouched
  so `cards_sidetable.json` needed no regeneration and its hash check
  (`CardSideTableTest`, inside `oracle_campaign_python_test`) is green on all
  five presets. **Replay: ZERO movement from the integration.** The standing
  corpus reproduces the followup branch's end state exactly — b45+b47
  `--- 12 file(s), 0 not clean ---`; G6-main `--- 30 file(s), 10 not clean ---`
  on the same standing rows at the same seqs (STS00221, STS00283, STS00462,
  STS00683, STS00856, STS01068, STS01221, STS01314, STS01372, STS02009 = the
  3 EVENT-offset, 4 stolen-gold, 2 FairyPotion and 1 unsimulated-grid rows);
  and the bottle seven `--- 7 file(s), 2 not clean ---` on the two standing
  gold-class rows alone (STS04925 seq 137, STS06578 seq 81), with STS00241
  CLEAN to terminal carrying its single escape-race record. **Nothing in the
  union moved a verdict** — expected, since the applied handoff changes only
  comments and provenance strings.

- **Wave-2: the four-track cleanup union proven on `wave2-integrate`** `[x]`
  — merges `wave2-harness` (3 commits, head `25c6449`), `wave2-engine` (8,
  `9561c8e`), `wave2-prov` (1, `663de42`) and `wave2-capture` (5, `33b086d`)
  onto master `e15ebad`, plus one integration commit (`d9cf143`,
  Orrery/Cauldron provenance re-derived). NOT yet on `master` — landing is the
  orchestrator's. **Track headlines:** harness — all four win/WSL preset
  families green, and one of the six parked `win-debug` failures was a REAL
  defect (`RunState`'s two undeclared padding gaps under `memcmp`, now explicit
  `pad_*` members + a layout-walk test); the `SHOP_ROOM`/`SHOP_SCREEN`
  `--replay` arms; `act_boss`; the archived-summary legacy read. engine —
  Blind+/Trip+ upgraded-target state divergence proven REAL and reproduced
  (`upgraded_target:` column, all three consumers at once); the Mayhem
  two-level queue fix from the recovered `MayhemPower$1` (`kPlayCardDeferRoll`);
  Explosive Potion re-typed pure/null-source; the purged-copy pool-row leak
  re-assessed and closed as the deliberate model; the Egg-trio onEquip gap
  found behind a stale "does not exist yet" comment. prov — 49 inner-class
  files recovered from the shipped jar (the tree is no longer lossy for the
  16); the nine out-of-range citations re-derived method-by-method (no
  mechanical offset existed; prose 9/9 MATCH). capture — the bottle grid's
  REVERSE master-deck order proven live end-to-end; the Courier restock
  measured live-exact over 13 purchases; the `NoBlockPower` game_id data fix.
  **Cross-track composition** (what a conflict-free merge would not have
  proven): the replay `main.cpp` touchpoint composed harness's
  moved-to-`command_map.hpp` resolvers with capture's two `--shop` fixes —
  `shop_choice_arg_to_index` now lives beside the shared resolvers and the
  `state` elision kept its call site; the obligations table carries every
  branch's rows with the cross-annotations applied (prov's Mayhem stop-the-line
  row DISCHARGED by `596f49e`; the nine-citations row's 22 handed-off lines
  landed by `9561c8e`; prov's a20 + Mayhem rows arrived joined on ONE physical
  table line and were split); `cards_sidetable.json` regenerated on the union
  cards.yaml (hash line only); `run_state.hpp`'s padding members and comment
  re-derivation merged disjoint, layout test green everywhere. **Verification:**
  WSL `debug`/`asan`/`release` AND `win-debug`/`win-asan` — full suite PASS, 0
  failed, all five; `fuzz_soak --seeds 300` failures: 0; stale-counts +
  doc-links clean. **The payoff run:** the six shop-gated bottle captures now
  walk their merchants — STS03244 CLEAN to its run terminal (the Bottled
  Lightning SKILL-grid descending proof, live), STS04888 CLEAN, and every
  divergence in the other four attributed to a standing class (mid-combat
  stolen-gold ×2, reconverging; the STS05143 Elixir HAND_SELECT class with
  STS03352 as a NEW second reproducer; and ONE new benign race-class
  transient — STS00241 seq 96, a Smoke Bomb escape the sim settles one record
  before the capture's mid-animation dump, zero-diff from seq 97 to terminal).
  The standing corpus reproduces its baselines exactly: b45+b47 twelve
  artifacts (ten CLEAN, two `Recall` stops, first divergence none anywhere)
  and the G6-main thirty (the same twelve attributed rows at the same seqs;
  STS00509 CLEAN at 171 records); the Courier `--shop` walk repeats stock
  clean 5 / purchase walks clean 5 / 0 divergences. Verdict tables:
  [`wavec_track2_replay_triage.md`](../tools/oracle_bridge/driver/wavec_track2_replay_triage.md)
  and [`wave2cap_capture_runbook.md`](../tools/oracle_bridge/driver/wave2cap_capture_runbook.md)
  §7. **Queued follow-up — the next wave's scope, owned, not lost:** the two
  sim-side (a)-candidates STS01789 seq 130 and STS00353 seq 97 (STS00241's
  Smoke Bomb transient is a related race-class third); STS05143 seq 51 Elixir
  HAND_SELECT (second reproducer STS03352 seq 143); `RecallOption` modelling
  (`RunState.keys` exists; run-layer call); the `RunState.boss_ids` run-layer
  mirror (act_boss row); the Egg-trio onEquip reward-screen preview pass (the
  equip-plumbing decision, now also named by Orrery's and Cauldron's corrected
  strings); the capture-gated power-`misc` and move-history blockers; and the
  eleven further +1000-class relics.yaml citations (new obligations row above).
...
- **Combat: pre-turn monster block clear + debuff duration ticks** `[x]` —
  ...
  - **`Vulnerable` and `Weak` never ticked down.** Both rows bound no hooks at
    ...
    same six lines as `FrailPower.java:40-49`, which was already native. All
...
### G6 `[x]` **Gate: S1 rules complete (M3)** — tag `g6-s1-content`
**Deps:** all B3.*, all B4.*
...
**Log:** Gate closure landed in commit `e15ebad` and is preserved by tag
`g6-s1-content`; G7 only repaired this stale unchecked heading.
