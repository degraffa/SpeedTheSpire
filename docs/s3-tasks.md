...
|---|---|---|---|
| Terminal drain filters actions queued by non-damage survivors too early | S3.62 source-order audit, 2026-09-07 | **S3.62** | **Repeated-clear repair provisionally accepted by S3.62a, 2026-09-15; live witness still owed.** The source-derived predicate distinguishes actual completed `clearPostCombatActions` calls and early returns. The original constructed Sentinel/Corruption/Panache play now retains source-traced energy 5 rather than 3, with HP/block unchanged. Initial-clear approximation and terminal-choice limits remain explicit; no live observation or gate closure is claimed. [Original audit](verification/s3-heart-terminal-order-audit.md), [repair and acceptance](verification/s3-62-terminal-clear-repair.md). |
...
| --- | --- | --- | --- |
| **Snecko's Eye/Confusion: a `MAKE_CARD` self-copy does not inherit the source card's Confusion-randomized cost** | S3.53 sweep (`_oracle_data/s3/s353_sweep.tsv`, 2026-09-03) | **DISCHARGED 2026-09-03 by costfix** | Witnessed in 3 of the 70 S2.V3 captures the sweep ran `--costs` over, all three carrying Snecko's Eye (`Confusion`): `s2v3_wave1_STS206243_ps5`, `s2v3_wave1_STS216263_ps95`, `s2v3_wave2_STS200527_ps9`. Root cause confirmed as suspected: Anger's `MAKE_CARD` self-copy (`registry/cards.yaml` id 11) reseeded the copy from the registry base row instead of `AbstractCard.makeStatEquivalentCopy` (AbstractCard.java:825-848), which also carries `cost`/`costForTurn`/`isCostModified`/`isCostModifiedForTurn`/`misc`/`freeToPlayOnce` — a Confusion-rolled Anger clones itself at the rolled cost, not 0. Fixed by a new `self_copy` registry bit (`CARD_MAKE_SELF_COPY_BIT`, `tools/registry_gen/stsgen/{steps,vocab}.py`) threading the played card's own pool index through the `MAKE_CARD` item (`interp.hpp` `kMakeCardSelfCopyBit`/source-index packing) to a shared `make_stat_equivalent_copy` helper (`interp_cards.cpp`) that copies exactly that field list — generalized to Dual Wield's `clone_card_to_hand`, which had the identical latent gap (a verbatim struct copy also drags `EXHAUST_ON_USE_ONCE`/`PURGE_ON_USE`/`AUTOPLAY_X_ENERGY` along; live-witnessed mid-fix via this capture's own Havoc-autoplayed Anger). All three named captures + the 90-capture sweep + all three committed corpora replay `--costs`/`--vitals`/`--replay` zero-diff after the fix |
| **Blood for Blood's `tookDamage` cost reduction under-counts by one HP-loss event relative to the capture, in the HAND** | S3.53 sweep (`_oracle_data/s3/s353_sweep.tsv`, 2026-09-03) | **DISCHARGED 2026-09-03 by costfix** | Witnessed in 2 of the 70 S2.V3 captures: `s2v3_wave2_STS227212_ps88` and `s2v3_wave2_STS228756_ps285`. The suspected `tookDamage()`-under-counting root cause was WRONG: `cards_took_player_damage` fires correctly on every positive in-combat HP-loss event and reduces every matching card it finds. Both witness records instead catch a card CREATED mid-fight by a played Discovery (a `CardRewardScreen.customCombatOpen` offer, translated as `screen_type: CARD_REWARD` — confirmed via the raw capture's `action_command`/`choice_list` sequence around the divergent seq). `BloodForBlood.makeCopy()` (BloodForBlood.java:60-67) pre-reduces every FRESHLY instantiated copy by `AbstractDungeon.player.damagedThisCombat`, clamped at 0 — a mechanism the engine's fresh-copy sites (`add_library_copy_to_hand`, `op_make_card`'s non-self-copy branch) never implemented. Fixed by a new `CombatState::damaged_this_combat` field (occupies the former `pad_player` byte, zero size/offset change; incremented in `cards_took_player_damage`, mirroring `AbstractPlayer.java:1465-1466`) and a shared `fresh_combat_card_cost` helper applied at both fresh-copy sites — covering Discovery (the live witness), Codex and Dead Branch, every combat-pool site that can mint a Blood for Blood. Both named captures + the 90-capture sweep + all three committed corpora replay `--costs` zero-diff after the fix |
| **A pre-existing `--replay` RunState divergence at an Act-3 double-boss `proceed` handoff, unrelated to cost or mask** | S3.53 sweep (`_oracle_data/s3/s353_sweep.tsv`, 2026-09-03) | **DISCHARGED 2026-09-03 by costfix — NOT A DEFECT** | Re-ran clean on current master + both fixes above: `s2v3_wave2_STS205404_ps20` still diverges identically at seq=894 floor=50 screen=COMPLETE cmd='proceed' (`hp: 36 -> 61`, `floor: 50 -> 51`, six `relics[i].counter` rows, `boss_ids[2]: 59 -> 58`), settling stale-vs-live as LIVE — but root-caused as a differ-tool reporting gap, not an engine defect. The engine's computed `boss_ids[2] == 58` (Awakened One) is independently CORRECT: the capture's own oracle dump at seq=893/894 (`game_state.oracle.encounterLists.boss`) reads `["Awakened One", "Donu and Deca"]`, i.e. the live game's own `bossList.get(0)` at this exact moment already names Awakened One as the next boss, matching the sim exactly. The apparent DIFF is `replay_run_diff`'s own A20-double-boss HANDOFF tolerance (main.cpp, `is_double_boss_handoff`) declining to engage: it requires a `k+1` capture record on `floor + 1` to compare against instead of this transitional one, and this specific capture's own final record IS seq=894 (`895 records compared ... stop: artifact exhausted`) — there is no k+1, because the capture was cut off exactly at the crossing. No code changed for this row; recorded as a verified non-issue rather than chased into the differ tool, which is a different subsystem than this task's card/power-body scope |
...
- **S3.62** `[~]` **Oracle campaigns: breadth + Act-4 depth.** The design §6
  ...
  **Inherited:** the S3.11 / S3.24 / S3.32 / S3.33 / S3.41 / S3.42 / S3.43
  ...
  (2026-09-07; repair S3.62a, 2026-09-15):** witness Sentinel exhaust energy
  before lethal Heart retaliation after the provisionally accepted repeated-clear repair; the
...
- **S3.62a** `[x]` **Source-backed terminal repeated-clear repair.** Bounded
  follow-up to S3.62's 2026-09-07 source-order audit. Distinguish source clearing
  actions and their early returns during the victory-terminal drain; preserve
  queue order and the existing initial partition, defeat and escape behavior.
  **Deps:** S3-G1; S3.62 source-order finding. **Acceptance:** complete-method
  source review, independent review, targeted six-preset engine/replay builds,
  existing committed corpus zero-diff with its negative controls, and explicitly
  constructed simulator observations. This provisional engineering acceptance
  does not discharge S3.62's live witness or S3-G2.
  **Inherited:** S3.62 retains the live terminal-energy/order and lethal
  retaliation witness; the deferred row above owns that obligation.
  **Log (2026-09-15):** A private pre-execution predicate now permits repeated
  filtering only when the represented source action reaches its clear and the
  monster group remains basically dead. UseCardAction's new Sentinel energy
  executes before Beat of Death. The independently rebuilt original probe
  changes energy 3 to 5 with HP 20/block 3 unchanged; six explicit variants run
  under win-release and ASan/UBSan with identical output. All six targeted
  builds and both-host three-corpus replay/cost/mask walks pass, including all
  injected controls. No unit tests, game launch or bridge deployment. Initial
  clear and generated-choice limits are recorded rather than certified.
  [Report, commands and external provenance](verification/s3-62-terminal-clear-repair.md).
...
- 2026-09-15 — scoped S3.62a for the source-backed terminal repeated-clear
  repair and provisional engineering acceptance. S3.62 keeps the live witness
  and all campaign obligations; no frozen mechanic or gate changed.
