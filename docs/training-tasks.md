...
| Complete the public-map planner's run-agent adapters | T3.3a, T3.3b | T3.3 | The bounded map primitive has explicit model callbacks and map-phase budgets. T3.3b adds a versioned public one-room transition/replay contract and a bounded real corpus under one scripted policy. Fitted public reveal/value adapters, shop/removal sequence search, imminent-fight combat invocation, other-phase budgets, special movement and multi-act adapters remain. Synthetic fixed-V and observational transition evidence do not establish stronger play; T3.4 dependencies are unchanged. |
...
- **T3.3** `[~]` ∥ **Run-level planner.** Sparse expectimax over the
  ...
  **Inherited (T3.3a–b):** complete the fitted public reveal/value model,
