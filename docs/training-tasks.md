...
| Complete the public-map planner's run-agent adapters | T3.3a–d | T3.3 | The bounded map primitive has explicit model callbacks and map-phase budgets; T3.3b adds a versioned public one-room transition/replay contract; T3.3c qualifies a development-only HP/death distribution and depth-one shadow adapter. T3.3d adds all-action branch/replay tooling but retires that exact route-use feasibility; retain source-corpus SHA verification before native replay or separately harden source interval bounds. Complete public reveal/value state, shop/removal sequence search, imminent-fight combat invocation, other-phase budgets, special movement and multi-act adapters remain. Synthetic, observational and shadow evidence do not establish stronger play; T3.4 dependencies are unchanged. |
...
- **T2.2** `[~]` **Combat ExIt loop v1.** From-scratch expert iteration on
  ...
  **2026-09-07 (T2.2g optional-hand encoding) — verified; quality bars unmet.**
  Encoding 2 exposes selected hand cards and optional CHOOSE source identity;
  strict companion/checkpoint versions prevent silent reinterpretation. All
  105,914 qualified rows reencode exactly under version 1; version 2 changes
  only 635 optional-hand rows. Debug/ASan/release builds, restored-state
  collision checks, instrumented actor collection/evaluation and independent
  legacy replay pass. Historical TinyNet workflows require an explicit
  encoding-1 build; the engine pin remains `019fa9f`.

  Both matched learners stop at index 480 (481 updates) and select zero; all
  starting tensors are preserved. Four complete frozen-suite evaluations give
  policy improvement bounds [-0.0039388604, +0.0048611396], lower-bound 99% CI
  [-0.0074894152, -0.0012], p=0.9978. The common 2,483-case conditional policy
  difference is exactly zero. Version 2 resolves five old caps but creates
  seven; original quality and retention bars remain unmet. Retain gen31 with
  encoding 1; no checkpoint promotion or further observation sweep.

  Saved-row audits distinguish residual target disagreement from FP16 aliasing
  (no additional collisions). A bounded real-row gradient audit measures
  weighted non-policy trunk gradients 5.59–17.59 times policy gradients,
  sometimes opposing them. T2.2h is the next separate bounded comparison:
  matched encoding-2 arms, standard control versus training-only detached
  pooled features for value/ensemble/HP/death heads, which still train.
  Preserve losses, targets, architecture, split, optimizer, budget and gates;
  verify zero forward/gradient routing on real rows first. Include zero in
  selection and predeclare futility when both selected states equal zero.
  T2.2 remains `[~]`; T2.3 and downstream dependencies remain unsatisfied.
  Reports under `SpireTrainer/docs/verification/`: `t2-2g-optional-hand-encoding.md`,
  ...
  **2026-09-07 (T2.2i teacher-configuration diagnosis) — verified; the
  collection teacher is weaker than deployed search.** One new evaluation
  of the historical 192-evaluation Gumbel-sequential-halving configuration
  at the exact gen31/encoding-1 checkpoint reused the bound v1-zero 48-PUCT
  reference after a hash-checked preflight; policy and all three scripted
  agents matched every reference CSV field, and an independent read-only
  review of the frozen driver and protocol found no blocker before launch.
  Teacher-minus-deployed mean bounds are [-0.0146369828, -0.0098369828],
  upper-bound 99% CI [-0.013196183614, -0.006593366998], improvement
  p=1.0: even the optimistic bound is negative. Teacher-minus-SIM_SEARCH is
  likewise robustly negative; the policy, blind and greedy contrasts cross
  zero. The teacher resolved all 2,500 cases (393 deaths versus 330 for
  deployed search, which capped 12). Fair-blind and SIM_SEARCH retention
  ratios are undefined because teacher search gain is negative; the
  original bars fail for both configurations. Measured 18,952 searched
  decisions / 23,495 engine steps in 91.5 s versus 21,580 / 26,610 in
  248.5 s; the eval output does not publish evaluator calls, so 192/48 are
  nominal budgets and algorithm and budget effects remain coupled. The
  weak-teacher hypothesis has direct support at this checkpoint, but no
  historical soft target is thereby judged. Retain gen31 encoding 1; no
  collection, promotion or engine change. T2.2 remains `[~]`. T2.2j next
  qualifies a matched 192-PUCT behavior (192 held fixed against this GSH
  result, scaling measured against 48-PUCT) before any collection-teacher
  algorithm change or new labels.
  Report under `SpireTrainer/docs/verification/`: `t2-2i-teacher-quality.md`.

  **2026-09-07 (T2.2j matched 192-PUCT) — verified; the algorithm, not the
  budget, explains the teacher deficit.** One new evaluation at the exact
  gen31/encoding-1 checkpoint held the nominal budget at 192 and switched
  selection from Gumbel sequential halving to PUCT, reusing both the bound
  48-PUCT reference and the T2.2i 192-GSH result after a hash-checked
  preflight; policy and all three scripted agents matched every CSV field
  across all three evaluations, and the orchestrator's read-only review of
  the frozen protocol, config and driver found no blocker before launch.
  Contrast A (puct192 minus gsh192) is robustly positive: mean bounds
  [+0.0118767264, +0.0154767264], lower-bound 99% CI
  [+0.008871588130, +0.015086319434], p=0.00005. Contrast B (puct192 minus
  puct48) is not: mean bounds [-0.0027602564, +0.0056397436], lower-bound
  99% CI [-0.006971522340, +0.000787775700], p=0.96075 — and not robustly
  negative either, so the sign of the 48-to-192 budget effect is
  undetermined. The preregistered `A_robust_B_not` branch holds. 192-PUCT
  is robustly above policy, blind and greedy but still below SIM_SEARCH,
  and the original bars fail for all three configurations; its retention
  ratio against `sim_search` is undefined (nonpositive gain) and both other
  60% margins are negative. It capped 9 cases, a strict subset of the 12
  capped by deployed search, with 327 deaths versus 330 and 393. Measured
  20,592 searched decisions / 25,616 engine steps in 795.2 s, versus
  18,952 / 23,495 in 91.5 s (GSH) and 21,580 / 26,610 in 248.5 s
  (48-PUCT); 192/192/48 remain nominal budgets, not evaluator-call
  counts. A teacher-algorithm change alone would therefore not lift
  target quality above deployed 48-PUCT, so the diagnosis moves to the
  value and policy heads. Retain gen31 encoding 1; no collection,
  ...
  **2026-09-08 (T2.2l disagreement atlas) — verified; collect targeted root
  evidence before choosing a learner change.** A frozen post-exploration
  analysis retained all 16,988 accepted T2.2k decision rows and localized
  2,277 of 3,690 disagreements to the fixed slice where the search-selected
  action has policy probability 0.10–<0.25. Its disagreement prevalence is
  0.3904 against 0.1267 outside (risk ratio 3.0826; clustered 99% interval
  [2.8111, 3.3969]), and its conservative episode-level SEARCH-minus-POLICY
  association has 99% lower endpoint 0.00775. The broad any-disagreement and
  early-disagreement patterns also have positive conservative lower endpoints;
  confident disagreements are too sparse to estimate and frequent
  disagreements have no positive association. Near ties miss their fixed 0.80
  point threshold. A deterministic manifest selects 256 unique replay states
  in four exact 64-state tranches, with all identities, action fields and
  preceding SEARCH-action prefixes rejoined to the accepted diagnostic. The
  run-4 receipt covers the exact final source tree and every result. Because
  the diagnostic has no raw visits, per-action backed-up sums, complete policy
  vector, search seeds, repeated-search stability or counterfactual outcomes,
  the branch is `targeted_root_evidence`; this association cannot identify a
  high-impact action or authorize training. T2.2 remains `[~]`.
  Report under `SpireTrainer/docs/verification/`:
  `t2-2l-disagreement-atlas.md`.

  **2026-09-08 (T2.2m targeted root evidence) — verified; the ordered branch
  is bounded forced-action rollouts, not a learner change.** Run 3 fixed the
  T2.2l 256-root cohort, engine pin, gen31/encoding-1 checkpoint and
  192-PUCT configuration. Native control/evidence summaries agree exactly
  across all 2,048 root/repeat identities; root arithmetic, reconstruction and
  read-back pass, and the 32-root Windows ASan/UBSan acceptance has no finding.
  The preserved run-1 evaluator-startup confound, v6 smoke refusal and run-2
  NumPy API refusal are excluded from scientific evidence. Mean visit advantage
  is 0.1762462120 (clustered 99% CI [0.1123403675, 0.2421977586]);
  `learner_ready` is 161/256 with lower bound 0.5427350427, below the required
  0.70. The `no_update` rule does not fire, `matched_learner` fails, and the
  ordered `forced_action_rollouts` branch holds. It authorizes only a fresh,
  preregistered forced SEARCH-versus-POLICY continuation from persisted roots
  under matched 192-PUCT; no training, checkpoint promotion, oracle claim or
  full-run quality claim follows. T2.2 remains `[~]`.
  Report under `SpireTrainer/docs/verification/`:
  ...
  **2026-09-08 (T2.2o endpoint resolution) — descriptive audit complete; no
  authorization changed.** A deterministic replay of the accepted T2.2n
  transcript restored all 4,096 archived episodes and verified all 29,704
  advances and 18,724 continuations. It emitted and directly compared the
  canonical `PublicView` endpoint bytes, independently re-derived the V0s
  route/key/value, and classified all 2,048 pairs: 1,163 same public endpoint,
  58 same scoring features, 434 same encoded key, 0 same score with different
  key, and 393 different score. All 200 all-score-tie roots split into 139
  HP-equal-only roots and 61 roots with an HP-different pair. The persisted
  32-root Windows ASan/UBSan subset has zero findings and exact Release
  endpoint, derivation, and classification equality. This is a selected-cohort
  decomposition only; T2.2 remains `[~]` and no training or promotion is
  authorized. Report under `SpireTrainer/docs/verification/`:
  `t2-2o-endpoint-resolution.md`.

  **2026-09-08 (T2.2p currency preflight) — preregistered and independently
  accepted; collection blocked.** The prospective current-pin experiment binds
  10,000 fresh seeds and both SIM_SEARCH policies, an event-based Act-1 label,
  complete-case paired six-versus-twelve-HP-quantile fits, exact missing-label
  sensitivity, and seven 99% qualification bars. The executable preflight
  verified ten lineage inputs, inventoried 24 declared intervals and 99,669
  explicit seed occurrences across 100 bounded JSON metadata files, and found
  zero overlap. It also exposed two blockers: cap-edge boss transitions are not
  latched immediately, and the historical fitter maps unresolved trajectories
  to zero. Only producer/evaluator hardening plus bounded real-run acceptance is
  next; the 20,000-trajectory cohort, fitting, holdout, registry, training and
  promotion remain unauthorized. T2.2 remains `[~]`. Report under
  `SpireTrainer/docs/verification/`: `t2-2p-currency-preflight.md`.

  **2026-09-08 (T2.2q currency pipeline) — producer/evaluator acceptance
  passed; registered collection is next.** The new Act-1 mode processes every
  transition immediately, stops on observed boss kill or prior death, retains
  typed unresolved reasons with NaN record targets, and writes train/dev/
  holdout digits to separate deterministic roots. Seed 1 proves the exact cap
  edge: cap 198 records the boss kill on advance 198, while cap 197 remains
  unresolved; Release and ASan/UBSan sidecars and shards match byte-for-byte.
  A staged evaluator pins the complete simulator stamp and record layout,
  validates labels and split identities, emits sealed train/dev predictions,
  consumes the holdout seal exactly once, and implements every registered
  estimator and bar. The independent review recomputed the tables, predictions
  and 20,000 bootstrap draws. Its 18-trajectory acceptance-only result is
  `scientific_use=false` and cannot qualify a currency. A separate execution
  task may now freeze and collect the registered cohort, but must stop before
  fitting or holdout access. T2.2 remains `[~]`. Report under
  `SpireTrainer/docs/verification/`: `t2-2q-currency-pipeline.md`.

  **2026-09-08 (T2.2r registered collection) — collection integrity passed;
  registered comparison stops early.** A clean landed producer and exclusive
  atomic receipt froze the registered population before any output root
  existed. Three primary and three repeat roots preserve the same 10,000 seeds
  and 20,000 trajectories; corresponding sidecars and shards are byte-identical
  across thread/chunk settings. Train/dev identities and typed targets pass,
  while holdout checks use only manifest metadata, file sizes and opaque
  digests. The 376 train and 48 dev cap-unresolved trajectories limit global
  resolution to at most 97.88%, below the registered 99% bar even if every
  held-out trajectory resolves. Independent review accepted collection
  integrity, chronology and holdout handling. The comparison stops early as
  `inconclusive_hold`; no fitting, holdout outcome access, training, currency
...
- **T3.3** `[~]` ∥ **Run-level planner.** Sparse expectimax over the
  ...
  **Inherited (T3.3a–d):** complete the fitted public reveal/value model,
  shop/removal and imminent-combat adapters, all-phase budget telemetry and
  special/multi-act movement obligations in the deferred table before claiming
  integration. The T3.3b transition corpus is observational under one scripted
  policy and does not provide counterfactual route values. T3.3d supplies
  paired one-world scripted continuations only; its route-use candidate is
  retired. Preserve source-corpus SHA verification before native replay or
  separately harden the inherited source reader's interval bounds.
  ...
  **Log (2026-09-08, T3.3b):** A versioned `.stsroom` producer now records the
  complete public action/hash prefix and exact public endpoints for each
  map-choice-to-next-map-choice room interval; a standalone replay tool
  independently reconstructs every canonical action, hash, endpoint and room
  boundary from `run_begin`. Frozen A20 Ironclad seeds `[3000001,3000065)`
  yielded 11,546 actions, 859 rooms and 795 complete transitions, with all 64
  Debug/ASan artifact pairs byte-identical and zero capped or unresolved runs.
  Independent source, format and native replay review passed. This establishes
  the transition contract only: no model was fitted and no route-quality,
  currency, training or promotion claim follows. T3.3 remains incomplete and
  T3.4 stays gated. Report: `SpireTrainer/docs/verification/t3-3b-room-transitions.md`.
  **Log (2026-09-08, T3.3c):** A frozen four-fold whole-seed development
  experiment fits a public room/floor/entry-HP hierarchy to T3.3b's 816
  eligible non-boss room outcomes. Candidate seed-equal Brier is 0.5917226973
  versus 0.6265915410 without entry HP, a 5.564844% relative improvement with
  99% whole-seed interval `[3.088652%, 8.175003%]`; its death-Brier change
  `+0.0004031` also passes the frozen guard. Native/Python parity, 1,280 direct
  planner expectations, Debug/ASan identity, and declared depth/boss refusals
  pass. The adapter is shadow-only, depth-one, HP/death-coarsened development
  evidence from already inspected data. It supplies no counterfactual route
  values, complete state, currency, policy-quality or full-run claim. T3.3
  remains incomplete and T3.4 stays gated. Report:
  `SpireTrainer/docs/verification/t3-3c-public-room-model.md`.

  **Log (2026-09-15, T3.3d recovery/integration):** Recovered the existing
  uncommitted experiment without repeating selection, collection or fitting.
  All 711 frozen bindings and 60 Debug/ASan artifact pairs match; fresh native
  replays reconstruct 127 legal branches with 1,833 advances, 117 next-map
  endings, 10 deaths, zero unresolved branches and 60 exact factual matches.
  Independent recomputation reproduces all 20,000 bootstrap draws. Contrast
  MSE improves 3.9833%, but its 99% interval [-2.2515%, 9.6018%] crosses zero;
  the exact route-use feasibility remains retired. Only four recommendation
  disagreements also miss the 16-seed interpretation minimum. Targeted Debug
  and ASan builds/replays and independent source review pass. The continuation
  is privileged SIM_SEARCH, not a qualified public-information actor. Model
  features remain public, the engine pin stays fixed, and no currency, policy,
  checkpoint, or full-run claim follows. T3.3 remains incomplete and T3.4 gated.
  Report: `SpireTrainer/docs/verification/t3-3d-route-counterfactuals.md`.
...
- 2026-09-15 — T3.3d recovers and lands the frozen all-route branch/replay
  ...
  candidate remains retired; T3.3/T3.4 statuses and the engine pin are unchanged.

- 2026-09-08 — T3.3c mirrors the qualified one-room public HP/death projection
  and depth-one shadow adapter on whole-seed development folds. The result
  enables a separately specified extension; T3.3 remains in progress and T3.4
  stays gated.

- 2026-09-08 — T3.3b mirrors the versioned public one-room transition and
  exact replay contract accepted on a frozen 64-seed real cohort. It enables a
  separately specified and frozen public-model task; T3.3 remains in progress
  and T3.4 stays gated.
...
- 2026-09-08 — T2.2r verifies the frozen registered cohort with deterministic
  ...
  prevents the registered global resolution bar from passing. Collection
  integrity is accepted; neither candidate is fitted or qualified.

- 2026-09-08 — T2.2q passes bounded producer/evaluator acceptance with an
  exact cap-edge witness, deterministic physical split repeats, a sealed
  train/dev fit, one-time holdout opening, and independent recomputation. It
  authorizes only a separately frozen execution of the registered cohort.

- 2026-09-08 — T2.2p freezes and passes the fresh current-pin currency
  preflight. Its seed range is metadata-disjoint, while two pipeline defects
  block collection until a bounded hardening task passes real-run acceptance.

- 2026-09-08 — T2.2o retires a deterministic descriptive endpoint and V0s
  resolution audit of the accepted T2.2n transcript. Full Release replay and
  the fixed ASan/UBSan subset passed, but the result does not alter
  `inconclusive_hold` or authorize training, promotion, or a next task.
...
- 2026-09-08 — T2.2m validates repeated native root evidence on the fixed 256
  ...
  visit advantage, selecting only the bounded `forced_action_rollouts` branch;
  no training or promotion is authorized.

- 2026-09-08 — T2.2l maps the accepted gen31 disagreement cohort and selects
  256 reproducible roots. The fixed 0.10–<0.25 search-action policy-probability
  slice contains 61.71% of disagreements and passes the descriptive
  concentration and episode-association bars, so the next step is repeated
  192-PUCT root evidence before any learner comparison.
...
- 2026-09-07 — T2.2j qualifies a matched 192-PUCT teacher at fixed gen31:
  ...
  48-to-192 budget contrast is not, and 192-PUCT still trails SIM_SEARCH.
  Keep the reference and original gates; diagnose the value and policy
  heads next.

- 2026-09-07 — T2.2i measures the historical 192-GSH collection teacher at
  fixed gen31 and finds it robustly weaker than deployed 48-PUCT on the
  frozen suite. Keep the reference and original gates; T2.2j next qualifies
  a matched 192-PUCT configuration before any collection-teacher change.
...
- 2026-09-07 — T2.2g completes the optional-hand observation ablation on
  ...
  tests non-policy shared-trunk gradient routing, motivated by recorded-row
  gradients; original quality bars, fixed pin and promotion gates stand.
