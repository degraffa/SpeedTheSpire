...
| the `Spire Heart` dialog (S3.31, the reserved non-pool id `kSpireHeartEventId`) | `0` | It is not an `EventId` at all, so it reaches the fall-through after the switch rather than an arm of it -- and the answer is right for the right reason: the body writes no scratch word (its whole state is `screen`, and every screen shows one always-enabled option). `event_id` and `screen` are carried as for any dialog; the room is `RoomType::Victory`, which `room_type` carries. |
