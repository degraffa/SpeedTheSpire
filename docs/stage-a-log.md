# Stage A Task Log (archive)

The verbatim task blocks of every **completed** Stage A task, moved out of
[stage-a-tasks.md](stage-a-tasks.md) so the ledger stays a compact index. Stage A
is finished (milestone **M1**, gate **G3**, tag `m1-walking-skeleton`), so this
file is **closed reference**: read it only when you need the provenance of a
landed Stage A mechanic (what exactly it shipped, which Java lines it cited,
what it measured, which stop-the-line correction it recorded). Nothing here is
a live instruction.

The frozen spec these tasks executed is [stage-a-design.md](stage-a-design.md);
the rules they worked under are now stated once in
[conventions.md](conventions.md), and Stage A's own wording of them is preserved
verbatim in the appendix at the bottom of this file.

Blocks are byte-for-byte as they stood in the ledger at the time of the split
(`docs: archive the Stage A ledger …`), in ledger order. Each is preceded by an
`<a id="…">` anchor (e.g. `#a43`) that the ledger's index entries link to.
...
## Appendix — Stage A's own working agreements (historical, verbatim)

Superseded by [conventions.md](conventions.md), which is the single
authoritative copy of these rules today. Kept here unaltered because the Logs
above cite them ("per the precedence rule", "stop-the-line", the gate tags) and
the Stage-A-era wording is what those tasks actually worked under.
