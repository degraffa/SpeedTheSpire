...
//     (combat_rewards.hpp).
//   * events / shops / rest sites: no room content (here: entering one reseeds
//     the floor streams then parks at
...
enum class RunPhase : uint8_t {
    ...
    // Value 7 is reserved for the independently-developed rest-site phase.
    // Parallel room-task coordination deliberately assigns treasure 8 so they
    // compose without either task renumbering a replay-visible enum.
    TREASURE_ROOM = 8,     // unopened Act-1 non-boss chest (open or skip).
};

inline constexpr uint8_t kReservedRestSiteRunPhaseValue = 7;
