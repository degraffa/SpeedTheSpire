...
// (AbstractPower p : this.powers) p.onDeath();`, AbstractMonster.java:741-746):
...
// immediately after (:747-750). Dispatched from the monster-death edge of
// op_damage / op_lose_hp (interp_damage.cpp), the same edge
// dispatch_relics_on_monster_death already fires from.
//
// TWO THINGS THE JAVA PINS HERE. `isDying` is set BEFORE the power walk (:743),
