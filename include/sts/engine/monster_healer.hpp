...
//   Healer.java:24-202; HealAction.java:13-38 -> AbstractMonster.heal
...
//
// THE MOVE TREE (getMove, Healer.java:156-183) reads a GROUP-WIDE HP SUM and is
...
//     Intent.BUFF)` (:161,165) -- BUFF, not DEFEND and not a heal-specific icon
//     -- and BUFF is `setMove((byte)3, Intent.BUFF)` (:179). So the intent alone
...
//     blocks (:159-167 and :170-178), not one branch, so a reading that assumes
...
//     that is neither isDying nor isEscaping (:104-107 / :114-117), in GROUP
...
//     RollMoveAction (:124), which sits OUTSIDE takeTurn's switch so all three
...
//     libGDX MathUtils -- playSfx a randomBoolean (:128) and playDeathSfx a
//     random(2) (:136) -- so neither the per-turn sounds nor the death cost a
//     seeded draw, and the Healer registers NO MonsterDieFn (contrast the
//     Mugger's, whose playDeathSfx is seeded aiRng).
//
// (6) ENC_NAME "HealerTank" IS DEAD CONTENT. The constant exists (:41) but no
...
// changeState's only key is "STAFF_RAISE" (:145-153), spine calls; damage()
// (:186-194) is super.damage plus the standard non-THORNS, output > 0 hit
// animation; die() (:196-201) is the unseeded sound plus animation. Nothing
...
// getMove's decision (Healer.java:156-183) as a pure function of the GROUP's HP,
