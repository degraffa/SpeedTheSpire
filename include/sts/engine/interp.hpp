...
// bit queues **addToBottom, behind it**. Those batches therefore leave the bit
// CLEAR and queue an explicit APPLY_POWER item immediately AFTER each spawn
// item at take-turn time, which resolves in exactly the addToTop position
// (the spawn's resolution queues nothing ahead of it, so "next item" and
// "top of queue at resolve" are the same slot). Reptomancer (S2.27) has the
// same shape.
