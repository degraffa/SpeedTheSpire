...
// UPGRADE_ALL's pile order / canUpgrade gate. Expected values are hand-computed
// from the cited decompiled Java (see the per-row provenance in
// registry/cards.yaml).
...
TEST(CardColorlessRaresRegistry, ExactRosterAndReservedGaps) {
    const std::array<std::pair<int, CardId>, 4> roster{{
    ...
    // 113-115, 117-119, 121-123, 125-126: owned by later B3.11 stages. A
    // reservation nothing asserts is indistinguishable from an omission.
    for (const int reserved : {113, 114, 115, 117, 118, 119,
                               121, 122, 123, 125, 126}) {
