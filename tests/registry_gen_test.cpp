...
TEST(RegistryGen, ManifestCounts) {
    ...
    EXPECT_EQ(m::kPowersCount, 60u);  // + S2.26's four Act-3 powers:
                                      // CONSTRICTED (102), FADING (103),
                                      // SHIFTING (104) and REACTIVE (105,
                                      // game_id "Compulsive" -- the class name
                                      // is not the join key). That batch's whole
                                      // PowerId grant, spent exactly; 96-101
                                      // belong to its concurrent siblings.
                                      ...
                                      // not this one's
    EXPECT_EQ(m::kMonstersCount, 38u); // + S2.26's four Act-3 "Beyond" normals:
                                       // Spire Growth (54, game_id "Serpent"),
                                       // Transient (55), Maw (56), Writhing Mass
                                       // (57) -- the 54-57 block spent exactly.
                                       // The batch's fifth monster, the Jaw Worm
                                       // Horde, added NO row: it reuses id 1
                                       // through a constructor variant.
